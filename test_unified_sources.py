    ...
    for source in sources:
        ...
        if kind == "socrata":
            ...
            if not ("FeatureServer" in base_url or "MapServer" in base_url):
                print(f"❌ ArcGIS source '{key}' base URL must point to FeatureServer/MapServer")
                return False
...
def test_arcgis_urls_format():
    ...
    for source in arcgis_sources:
        ...
        if not (base_url.endswith("/FeatureServer/0") or base_url.endswith("/MapServer/0")):
            print(f"❌ ArcGIS source '{key}' should end with /FeatureServer/0 or /MapServer/0")
            return False
