...
class IngestionRunner:
    ...
    async def _load_source_raw(
        self, source: Dict[str, Any], since: Optional[datetime]
    ) -> IngestState:
        ...
        if source_kind == "tpia":
            self.logger.info(
                f"Skipping TPIA source {source_id} - manual processing required"
            )
            return IngestState(
                source_id=source_id,
                last_updated_seen=None,
        ...
        if records:
            ...
            timestamps = [
                r.get(source["updated_field"])
                for r in records
                if r.get(source["updated_field"])
            ]
            if timestamps:
    ...
    async def normalize(self) -> Dict[str, Any]:
        ...
        results = {"normalized_sources": [], "total_records": 0, "errors": []}
        ...
        for source in self.config.get("sources", []):
            try:
                ...
                results["errors"].append({"source_id": source["id"], "error": str(e)})

        self.logger.info(
            f"Normalization completed for {len(results['normalized_sources'])} sources"
        )
        return results
    ...
    async def publish(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["errors"].append({"entity": "lead_scoring", "error": str(e)})

        self.logger.info(
            f"Publishing completed for {len(results['published_entities'])} entities"
        )
        return results
...
async def main():
    ...
    parser.add_argument(
        "command",
        choices=["load_raw", "normalize", "publish"],
        help="Command to execute",
    )
    parser.add_argument(
        "--since", type=str, help="Timestamp to load data since (ISO format)"
    )
    parser.add_argument(
        "--config",
        type=str,
