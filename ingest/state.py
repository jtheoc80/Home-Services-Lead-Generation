        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    conn.commit()
                    logger.info(
                        f"Updated ingest state for {source_id}: status={status}"
                    )
                    return True
        except Exception as e:
...
def update_last_updated_seen(
    source_id: str, timestamp: datetime, status: str = "success"
) -> bool:
...
if __name__ == "__main__":
    ...
    if command == "list":
        ...
        for source_id, state in states.items():
            print(
                f"  {source_id}: last_run={state.get('last_run_at')}, status={state.get('last_status')}"
            )
