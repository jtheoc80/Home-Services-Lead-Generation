...
def fetch(domain, dataset, where=None, limit=50000, offset=0, app_token=None):
    ...
    # Build parameters
    params = {"$limit": min(limit, 50000), "$offset": offset}  # Socrata max
...
class SocrataConnector:
    ...
    def get_paginated_records(
        ...
        while True:
            ...
            logger.info(
                f"Fetching batch {offset // self.page_size + 1}, " f"offset {offset}"
            )
    ...
    def get_records_by_date_range(
        self, start_date: datetime, end_date: datetime, date_field: Optional[str] = None
    ) -> Iterator[Dict[str, Any]]:
    ...
    def search_records(self, query: str) -> Iterator[Dict[str, Any]]:
        ...
        params = {"$q": query, "$limit": self.page_size, "$offset": 0}
...
if __name__ == "__main__":
    ...
    def query_data(
        ...
        Returns:
            List of records
        """
        params = {"$limit": min(limit, 50000), "$offset": offset}  # Socrata max limit
    ...
    def _normalize_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        normalized = {"raw_data": record, "source_type": "socrata"}
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(
                metadata_url, params=params, timeout=self.timeout
            )
            response.raise_for_status()
...
def create_socrata_connector(source_config: Dict[str, Any]) -> SocrataConnector:
    ...
    return SocrataConnector(
        base_url=endpoint, app_token=app_token, timeout=timeout, retry_count=retry_count
    )
