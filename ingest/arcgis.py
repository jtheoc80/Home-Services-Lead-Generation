    ...
    for attempt in range(max_retries):
        try:
            response = requests.get(
                query_url, params=params, headers=headers, timeout=30
            )
...
class ArcGISConnector:
    ...
    def get_paginated_features(
        self, where_clause: str = "1=1"
        ...
        while offset < total_count:
            ...
            params.update({"where": where_clause, "resultOffset": offset})

            logger.info(
                f"Fetching batch {offset // self.page_size + 1}, "
        ...
        while True:
            try:
                ...
                if (
                    ...
                    and not response["exceededTransferLimit"]
                ):
                    break
...
def create_arcgis_connector(source_config: Dict[str, Any]) -> ArcGISConnector:
    ...
    return ArcGISConnector(base_url=endpoint, timeout=timeout, retry_count=retry_count)
