...
def generate_sample_leads() -> List[PermitRecord]:
    ...
    for i in range(10):
        ...
        permit = PermitRecord(
            ...
            address=addresses[jurisdiction["slug"]][
                i % len(addresses[jurisdiction["slug"]])
            ],
        ...
        logger.info(
            f"Generated high-value lead {i+1}: {permit.permit_id} - {permit.work_class} (${permit.value:,}) in {permit.jurisdiction}"
        )
...
def push_to_permits_tables(permits: List[PermitRecord]) -> Dict[str, int]:
    ...
    if not os.getenv("SUPABASE_URL") or not os.getenv("SUPABASE_SERVICE_ROLE_KEY"):
        logger.warning(
            "Supabase credentials not set. Would push to permits tables here."
        )
    ...
    for jurisdiction, jurisdiction_permits in jurisdiction_groups.items():
        ...
        try:
            sink = SupabaseSink(
                upsert_table=table_name, conflict_col="event_id", chunk_size=10
            )
...
def transform_to_leads(permits: List[PermitRecord]) -> List[Dict[str, Any]]:
    ...
    for permit in permits:
        ...
        lead = {
            ...
            "county": (
                permit.jurisdiction.replace(" County", "")
        ...
        leads.append(lead)
        logger.info(
            f"Transformed permit {permit.permit_id} to lead: {lead['name']} - {lead['service']} (Score: {lead['lead_score']})"
        )
...
def map_work_to_service(work_class: str) -> str:
    ...
    if any(
        keyword in work_lower
        for keyword in ["kitchen", "bath", "remodel", "renovation"]
    ):
        return "Home Remodeling"
    elif any(
        keyword in work_lower for keyword in ["hvac", "air", "heating", "cooling"]
    ):
...
def main():
    ...
    for jurisdiction, stats in sorted(jurisdictions.items()):
        logger.info(
            f"  - {jurisdiction}: {stats['count']} permits (${stats['value']:,} total value)"
        )
    ...
    logger.info(
        f"Raw permits push: {permits_result['success']} success, {permits_result['failed']} failed"
    )
    ...
    logger.info(
        f"Leads table push: {leads_result['success']} success, {leads_result['failed']} failed"
    )
    ...
    for i, lead in enumerate(top_leads, 1):
        logger.info(
            f"  {i}. {lead['name']} - {lead['service']} - ${lead['value']:,} - {lead['score_label']} ({lead['lead_score']})"
        )
    ...
    logger.info(
        f"   📋 Generated {len(permits)} high-value permit leads from 4 Texas counties"
    )
    logger.info(f"   💰 Total project value: ${total_value:,}")
    logger.info(
        f"   🗄️  Pushed to raw permits tables: {permits_result['success']}/{len(permits)}"
    )
    logger.info(f"   🎯 Transformed to customer leads: {len(leads)}")
    logger.info(
        f"   📊 Pushed to leads dashboard table: {leads_result['success']}/{len(leads)}"
    )
