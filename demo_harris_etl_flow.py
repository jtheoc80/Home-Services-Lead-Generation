...
from permit_leads.adapters.storage import Storage
...
def demonstrate_etl_flow():
    ...
    since_timestamp = adapter.etl_state.get_since_timestamp(adapter.source_name, fallback_days=7)
    logger.info(f"   ✅ Will query permits with ISSUEDDATE > {since_timestamp}")
    logger.info(f"   ℹ️  Note: This includes a 1-minute buffer to prevent gaps")
    ...
    if upsert_success:
        ...
        if success:
            logger.info(f"   ✅ ETL state updated successfully for {adapter.source_name}")
        else:
    ...
    if upsert_success:
        ...
        if new_last_run:
            ...
            logger.info("   ⚠️  Could not retrieve updated timestamp (database not available)")
