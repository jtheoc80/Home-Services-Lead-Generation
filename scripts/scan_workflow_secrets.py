...
class WorkflowSecretsScanner:
    ...
    def _find_patterns_in_line(self, file_path: Path, line: str, line_num: int, all_lines: List[str]):
    ...
    def _analyze_yaml_structure(self, file_path: Path, yaml_content: dict, raw_content: str):
    ...
    def _determine_if_required(self, line: str, secret_name: str, line_num: int, all_lines: List[str]) -> bool:
        """
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"- **Files scanned:** {len(list(self.workflows_dir.glob('*.yml')) + list(self.workflows_dir.glob('*.yaml')))}")
        ...
        for pattern_type in ['supabase', 'permits_url', 'vercel_token', 'railway_token']:
            count = len([u for u in self.secret_usages if u.pattern_type == pattern_type])
            report.append(f"- **{pattern_type.replace('_', ' ').title()}:** {count} usages")
        report.append("")
        ...
        for secret in sorted(self.get_unique_secrets()):
            ...
            is_required = any(u.is_required for u in self.secret_usages if u.secret_name == secret)
            status = "Required" if is_required else "Optional"
            files_count = len(set(u.file_path for u in self.secret_usages if u.secret_name == secret))
            report.append(f"- `{secret}` - **{status}** (used in {files_count} file{'s' if files_count > 1 else ''})")

        return "\n".join(report)


def main():
    parser = argparse.ArgumentParser(description="Scan GitHub workflow files for secret patterns")
    parser.add_argument(
        "--workflows-dir", 
