...
def check_supabase_health(supabase_url, service_role_key):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if response.status == 200:
                log_success(
                    f"Supabase service role authentication OK ({query_latency:.0f}ms)"
                )
                return True
            else:
                log_error(
                    f"Supabase service role query failed with status {response.status}"
                )
                return False
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            if response.status in [200, 301, 302]:
                log_success(
                    f"{name} OK ({latency:.0f}ms)"
                    if response.status == 200
                    else f"{name} redirected (status {response.status})"
                )
                return True, True
            else:
...
def main():
    ...
    optional_vars = ["HOUSTON_SOLD_PERMITS_URL"]
    ...
    if "HOUSTON_SOLD_PERMITS_URL" in env_vars:
        url_checks.append(
            (env_vars["HOUSTON_SOLD_PERMITS_URL"], "HOUSTON_SOLD_PERMITS_URL")
        )
    ...
    supabase_ok = check_supabase_health(
        env_vars["SUPABASE_URL"], env_vars["SUPABASE_SERVICE_ROLE_KEY"]
    )
    ...
    all_houston_reachable = (
        houston_weekly_reachable
        and houston_sold_reachable
        and houston_website_reachable
    )
    if all_houston_reachable:
        ...
        log_info(
            "   This is not critical - the ETL may still work with cached/alternative data"
        )

    # Only fail on critical issues (Supabase)
    if supabase_ok:
        log_success(
            "🎉 Critical preflight checks passed! Ready to run City of Houston ETL."
        )
        log_info("📚 For more information, see: docs/coh-etl-script.md")
        if not all_houston_reachable:
            log_warning(
                "⚠️  Note: Some Houston endpoints are unreachable but ETL will proceed"
            )
            log_info(
                "💡 If connectivity issues persist, consider allowlisting www.houstontx.gov"
            )
        return 0
    else:
