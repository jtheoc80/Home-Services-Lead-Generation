...
def parse_time_string(time_str):
    ...
    if not match:
        raise ValueError(f"Invalid time format: {time_str}. Use format like 3d, 24h, 5m")
    ...
    if unit == 'd':
        ...
        return max(1, math.ceil(num / (24 * 60)))  # minutes to days (round up, at least 1)
    else:
        raise ValueError(f"Unknown time unit: {unit}")


def main():
    parser = argparse.ArgumentParser(description='Dallas County permit scraping wrapper')
    parser.add_argument('--since', default='7d', help='Time period (e.g., 3d, 24h, 5m)')
    parser.add_argument('--verbose', '-v', action='store_true', help='Enable verbose logging')
    parser.add_argument('--dry-run', action='store_true', help='Dry run mode')
    parser.add_argument('--sample', action='store_true', help='Use sample data')
    parser.add_argument('--retries', type=int, default=3, help='Number of retries')
    parser.add_argument('--formats', nargs='+', default=['csv', 'sqlite', 'jsonl'], 
                       help='Output formats')
