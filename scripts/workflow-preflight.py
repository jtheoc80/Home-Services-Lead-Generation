    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            if response.status == 200:
                ...
                log_warning(
                    f"Harris County permits endpoint returned status {response.status}"
                )
...
def main():
    ...
    if supabase_ok and harris_ok:
        log_success(
            "🎉 All preflight checks passed! Ready to run Harris County permit scraper."
        )
        log_info("📚 For more information, see: docs/workflows-secrets.md")
        return 0
    else:
        log_error(
            "💥 Preflight checks failed! Please resolve the above issues before running the scraper."
        )
