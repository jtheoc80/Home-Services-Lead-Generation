    ...
    def validate_vercel(self) -> bool:
        ...
        for var in server_secrets:
            ...
            if not value:
                ...
                self.errors.append(f"Server secret {var} incorrectly prefixed with NEXT_PUBLIC_")
            else:
        ...
        for var in browser_safe:
            ...
            if not value:
                ...
                self.errors.append(f"Browser-safe variable {var} must start with NEXT_PUBLIC_")
            else:
                self._validate_specific_var(var, value)
    ...
    def _validate_specific_var(self, var_name: str, value: str) -> None:
        ...
        if var_name == "DATABASE_URL":
            if not value.startswith(("postgresql://", "postgres://")):
                self.errors.append(f"{var_name} must start with postgresql:// or postgres://")
            else:
                ...
                if not parsed.password:
                    self.warnings.append(f"{var_name} missing password (might be required)")
        ...
        elif var_name in ["STRIPE_PUBLISHABLE_KEY", "NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY"]:
            ...
            if not value.startswith("AC"):
                self.warnings.append(f"{var_name} doesn't look like a Twilio Account SID")
...
def main():
    ...
    parser.add_argument('--mode', choices=['vercel', 'railway', 'ingestion', 'all'],
