...
def check_last_run(args):
    ...
    if last_run:
        ...
        since = state_manager.get_since_timestamp(
            args.source, fallback_days=args.fallback_days
        )
...
def update_last_run(args):
    ...
    if args.timestamp:
        ...
        try:
            ...
            print(
                f"Error: Invalid timestamp format '{args.timestamp}'. Use ISO format like '2025-01-15T10:30:00'"
            )
            return
    else:
        # Use current time
...
def reset_source(args):
    ...
    if not args.confirm:
        print(
            f"This will reset ETL state for '{args.source}'. Use --confirm to proceed."
        )
        return
    ...
    print(
        "Note: Reset functionality requires implementation of delete operation in ETL state manager"
    )
...
""",
    ...
    check_parser = subparsers.add_parser(
        "check", help="Check last run timestamp for a source"
    )
    check_parser.add_argument(
        "source", help="Source name (e.g., harris_issued_permits)"
    )
    check_parser.add_argument(
        "--fallback-days",
        type=int,
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update last run timestamp for a source"
    )
    update_parser.add_argument(
        "source", help="Source name (e.g., harris_issued_permits)"
    )
    update_parser.add_argument(
        "--timestamp", help="Specific timestamp (ISO format), current time if omitted"
    )
    update_parser.set_defaults(func=update_last_run)

    # List command
    list_parser = subparsers.add_parser(
        "list", help="List all sources and their last run timestamps"
    )
