...
class ETLStateManager:
        ...
        try:
            ...
            if response.data and response.data.get("last_run"):
                last_run = datetime.fromisoformat(
                    response.data["last_run"].replace("Z", "+00:00")
                )
    ...
    def update_state(
        ...
        try:
            ...
            response = (
                self.client.table("etl_state")
                .upsert({**update_data, "source": source}, on_conflict="source")
                .execute()
            )
    ...
    def list_all_sources(self) -> list:
        ...
        try:
            response = (
                self.client.table("etl_state").select("*").order("source").execute()
            )
            return response.data
        except Exception as e:
    ...
    def initialize_source(self, source: str) -> bool:
        ...
        try:
            self.client.table("etl_state").upsert(
                {"source": source, "status": "unknown", "records_processed": 0},
                on_conflict="source",
            ).execute()
            return True
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "command", choices=["get", "update", "list", "init"], help="Command to execute"
    )
    ...
    parser.add_argument(
        "--records", type=int, default=0, help="Number of records processed"
    )
    parser.add_argument("--error", help="Error message for failed runs")
    parser.add_argument(
        "--overlap", type=int, default=1, help="Overlap minutes for get command"
    )
    ...
    try:
        ...
        if args.command == "get":
            ...
            success = manager.update_state(
                args.source, args.status, args.records, args.error
            )
            if success:
            ...
            for source in sources:
                last_run = (
                    source.get("last_run", "Never")[:19]
                    if source.get("last_run")
                    else "Never"
                )
                print(
                    f"{source['source']:<25} {source['status']:<10} {last_run:<20} {source['records_processed']}"
                )
