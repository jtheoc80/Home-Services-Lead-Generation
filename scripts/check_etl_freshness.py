...
def main():
    ...
    try:
        ...
        req.add_header('Prefer', 'count=exact')  # Request exact count in content-range header
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            if content_range:
                ...
                if '/' in content_range:
                    ...
                    try:
                        count = int(count_str)
                        print(f"✅ ETL freshness check complete: {count} permits issued in last 24 hours")
                        print(f"📊 Count from content-range header: {count}")
                        return count
                    except ValueError:
                        print(f"⚠️  Could not parse count from content-range: {content_range}")
                        return 0
                else:
