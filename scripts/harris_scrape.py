...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Harris County permit scraper (fallback)"
    )
    parser.add_argument(
        "--days", type=int, default=1, help="Days back to scrape (default: 1)"
    )
    ...
    try:
        ...
        try:
            result = subprocess.run(
                cmd, cwd=project_root, capture_output=False, check=True
            )
            logger.info("Harris County permit scraping completed successfully")
            ...
            logger.error(
                f"Harris County permit scraping failed with exit code {e.returncode}"
            )
            return e.returncode
