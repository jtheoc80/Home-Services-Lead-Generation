...
class PermitsValidationSuite:
    ...
    def validate_permits_data(
        self, data_path: str = "data/yesterday.csv"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "jurisdiction" in df.columns:
                ...
                invalid_jurisdictions = df[
                    ~df["jurisdiction"].isin(valid_jurisdictions)
                ]["jurisdiction"].unique()
                if len(invalid_jurisdictions) == 0:
                    expectations.append("jurisdiction_valid")
                else:
                    failures.append(
                        f"invalid_jurisdictions: {list(invalid_jurisdictions)}"
                    )
            else:
            ...
            if success:
                logger.info(
                    f"Permits validation PASSED: {successful_expectations}/{total_expectations} checks"
                )
            else:
                logger.warning(
                    f"Permits validation FAILED: {failed_expectations}/{total_expectations} checks failed"
                )
    ...
    def _add_permits_expectations(self, suite, context):
        ...
        try:
            ...
            suite.add_expectation(gx.expectations.ExpectColumnToExist(column="address"))
            suite.add_expectation(
    ...
    try:
        ...
        try:
            data_asset = datasource.add_csv_asset(
                asset_name, filepath_or_buffer=data_path
            )
        except Exception:
...
def run_permits_checkpoint(data_path: str = "data/yesterday.csv") -> Dict[str, Any]:
    ...
    try:
        ...
        validation_results = {
            ...
            "run_name": "permits_validation_run",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run data quality validation on permits data"
    )
    parser.add_argument(
        "--data-path", default="data/yesterday.csv", help="Path to CSV data file"
    )
