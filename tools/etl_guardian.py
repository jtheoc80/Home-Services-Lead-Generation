...
def ensure_mkdir_step(workflow: Dict[str, Any]) -> bool:
    ...
    for i, step in enumerate(steps):
        if "name" in step and (
            "install deps" in step["name"].lower()
            or "setup python" in step["name"].lower()
        ):
            insert_index = i + 1

    # Add the mkdir step
    mkdir_step = {"name": "Ensure output dirs", "run": "mkdir -p logs artifacts data"}
...
def ensure_concurrency_guard(workflow: Dict[str, Any]) -> bool:
    ...
    expected_concurrency = {"group": "nightly-etl", "cancel-in-progress": False}

    current_concurrency = etl_job.get("concurrency", {})

    if (
        current_concurrency.get("group") != expected_concurrency["group"]
        or current_concurrency.get("cancel-in-progress")
        != expected_concurrency["cancel-in-progress"]
    ):
...
def collapse_artifact_uploaders(workflow: Dict[str, Any]) -> bool:
    ...
    new_steps = [
        step
        for i, step in enumerate(steps)
        if i not in [idx for idx, _ in upload_steps]
    ]
    ...
    consolidated_upload = {
        ...
        "with": {
            ...
            "path": ["permit_leads/artifacts/**/*.csv", "permit_leads/logs/**/*.log"],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ETL Guardian - Automatic ETL Workflow Fixer"
    )
    parser.add_argument(
        "--write",
        action="store_true",
