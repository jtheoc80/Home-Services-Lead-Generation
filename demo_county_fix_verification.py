...
def simulate_rpc_call() -> Dict[str, int]:
    ...
    result = {"inserted_count": 15, "updated_count": 3, "total_processed": 18}

    print("✅ RPC completed successfully!")
    print(
        f"   📊 Results: {result['inserted_count']} inserted, {result['updated_count']} updated, {result['total_processed']} total"
    )
    return result
...
def verify_no_nulls() -> Dict[str, Any]:
    ...
    if verification["missing_permit_id_count"] == 0:
        ...
        print(
            f"   ❌ Found {verification['missing_permit_id_count']} leads without permit_id keys"
        )
...
def show_sample_leads() -> None:
    ...
    for i, lead in enumerate(sample_leads, 1):
        ...
        print(
            f"      Has key: {'✅' if lead['permit_id'] else '❌'}, Created: {lead['created_at']}"
        )
        print()
...
def main():
    ...
    all_good = (
        verification["null_county_count"] == 0
        and verification["null_name_count"] == 0
        and verification["missing_permit_id_count"] == 0
    )
    ...
    print(
        "   1. Apply migration: psql -f supabase/migrations/20250107000000_fix_county_nulls.sql"
    )
