...
class TestTXPermitsIntegration(unittest.TestCase):
    ...
    def test_scoring_v0_pool_permit(self):
        ...
        self.assertGreater(
            result["score"], 50, "Pool installations should score moderately"
        )
    ...
    def test_field_aliases_coverage(self):
        ...
        for field in required_fields:
            self.assertIn(
                field, PERMIT_ALIASES, f"Missing alias configuration for {field}"
            )
            self.assertIsInstance(
                PERMIT_ALIASES[field], list, f"Aliases for {field} should be a list"
            )
            self.assertGreater(
                len(PERMIT_ALIASES[field]), 0, f"No aliases defined for {field}"
            )
    ...
    def test_coordinate_validation(self):
        ...
        self.assertIsNotNone(
            normalized["geom"], "Valid TX coordinates should create geometry"
        )
        ...
        self.assertIsNone(
            normalized_invalid["geom"], "Invalid coordinates should not create geometry"
        )
