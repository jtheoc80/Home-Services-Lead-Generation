...
try:
    from backend.app.demand_forecast import (
        ForecastConfig, SurgeLabeler, FeatureEngineer, 
        DemandSurgeForecaster, get_demand_surge_forecaster
    )
    from reports.impact_projection import get_impact_projection_reporter, generate_example_report
    ...
    class ForecastConfig:
        def __init__(self, **kwargs):
            self.surge_percentile_threshold = kwargs.get('surge_percentile_threshold', 90.0)
...
def test_surge_labeler():
    ...
    try:
        ...
        dates = pd.date_range(start=date.today() - timedelta(weeks=52), 
                             end=date.today(), freq='W-MON')
        ...
        mock_data['is_surge'] = mock_data['percentile_rank'] >= config.surge_percentile_threshold
        
        surge_weeks = mock_data['is_surge'].sum()
        expected_surge_weeks = len(mock_data) * (1 - config.surge_percentile_threshold / 100)
        
        logger.info(f"Mock data created: {len(mock_data)} weeks, {surge_weeks} surge weeks")
...
def test_feature_engineer():
    ...
    try:
        ...
        mock_permits = pd.DataFrame({
            'date': dates,
            'permit_count': permit_counts
        })
        ...
        for lag in config.feature_lags:
            features_df[f'permits_lag_{lag}w'] = features_df['permit_count'].shift(lag * 7)
        
        # Add mock moving averages
        for window in config.rolling_windows:
            features_df[f'permits_ma_{window}w'] = features_df['permit_count'].rolling(window=window*7, min_periods=1).mean()
...
def test_model_training():
    ...
    try:
        ...
        gb_model = GradientBoostingClassifier(
            n_estimators=20,  # Reduce for speed
            max_depth=3,
            random_state=42
        )
...
def test_report_generation():
    ...
    try:
        ...
        report = {
            ...
            "visualizations": {
                "summary_table": sample_data.to_dict(orient='records')
            },
...
def test_api_integration():
    ...
    try:
        ...
        logger.info(f"Mock API response: {mock_response.region} score={mock_response.score}")
...
def main():
    ...
    parser.add_argument('--component', choices=['config', 'labeler', 'features', 'models', 'reports', 'api'],
