...
class DataValidationSuite:
        ...
        for i, record in enumerate(data):
            ...
            if issued_date:
                if not self._is_valid_date(issued_date):
                    errors.append(
                        f"Record {i}: Invalid date format for 'issued_date': {issued_date}"
                    )
            ...
            if lat and lon:
                if not self._is_valid_texas_coordinates(lat, lon):
                    warnings.append(
                        f"Record {i}: Coordinates outside Texas bounds: [REDACTED]"
                    )

        # Check for duplicates
        permit_numbers = [
            r.get("permit_number") for r in data if r.get("permit_number")
        ]
        duplicates = self._find_duplicates(permit_numbers)
        if duplicates:
            warnings.extend(
                [f"Duplicate permit numbers found: {dup}" for dup in duplicates[:5]]
            )
        ...
        return self._create_result(
            ...
            metrics={
                ...
                "records_with_coordinates": sum(
                    1 for r in data if r.get("latitude") and r.get("longitude")
                ),
    ...
    def _validate_violations(
        ...
        for i, record in enumerate(data):
            ...
            if created_date and not self._is_valid_date(created_date):
                errors.append(
                    f"Record {i}: Invalid date format for 'created_date': {created_date}"
                )
            ...
            if lat and lon:
                if not self._is_valid_texas_coordinates(lat, lon):
                    warnings.append(
                        f"Record {i}: Coordinates outside Texas bounds: {lat}, {lon}"
                    )
        ...
        if duplicates:
            warnings.extend(
                [f"Duplicate case numbers found: {dup}" for dup in duplicates[:5]]
            )
    ...
    def _validate_inspections(
        ...
        for i, record in enumerate(data):
            ...
            if inspection_date and not self._is_valid_date(inspection_date):
                errors.append(
                    f"Record {i}: Invalid date format for 'inspection_date': {inspection_date}"
                )
    ...
    def _validate_awards(
        ...
        for i, record in enumerate(data):
            ...
            if award_date and not self._is_valid_date(award_date):
                errors.append(
                    f"Record {i}: Invalid date format for 'award_date': {award_date}"
                )
    ...
    def _validate_contractors(
        ...
        license_numbers = [
            r.get("license_number") for r in data if r.get("license_number")
        ]
        duplicates = self._find_duplicates(license_numbers)
        if duplicates:
            warnings.extend(
                [f"Duplicate license numbers found: {dup}" for dup in duplicates[:5]]
            )
        ...
        for i, record in enumerate(data):
            for date_field in ["issue_date", "expiration_date"]:
                ...
                if date_value and not self._is_valid_date(date_value):
                    warnings.append(
                        f"Record {i}: Invalid date format for '{date_field}': {date_value}"
                    )
    ...
    def validate_all_raw_data(self) -> Dict[str, Any]:
        ...
        for json_file in raw_dir.glob("*.json"):
            ...
            if result["success"]:
                ...
                sanitized_message = result["message"].replace(
                    "Coordinates outside Texas bounds:",
                    "Coordinates outside Texas bounds: [REDACTED]",
                )
            ...
            try:
                ...
                for i, record in enumerate(data):
                    ...
                    for field in required_fields:
                        if field not in record:
                            errors.append(
                                f"Record {i}: Missing normalized field '{field}'"
                            )
                    ...
                    if quality_score is not None:
                        if not isinstance(quality_score, (int, float)) or not (
                            ...
                            warnings.append(
                                f"Record {i}: Invalid quality score: {quality_score}"
                            )
                ...
                result = self._create_result(
                    ...
                    metrics={
                        "total_records": len(data),
                        "avg_quality_score": (
                            sum(r.get("quality_score", 0) for r in data) / len(data)
                            if data
                            else 0
                        ),
                    },
                ...
                result = self._create_result(
                    str(json_file), False, f"Validation error: {str(e)}"
                )
                results.append(result)
