...
def test_trace_api_endpoint(trace_id: str, debug_key: str = "test-debug-key"):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(
                f"✓ API returned {data.get('total_logs', 0)} logs for trace {trace_id}"
            )
            return True
        elif response.status_code == 401:
            print("! API returned 401 - debug key might not be configured")
            return False
        else:
...
def test_direct_logging():
    ...
    try:
        ...
        success = log_ingest_step(
            trace_id, "test_stage", True, {"test_data": "success"}
        )
        if success:
        ...
        # Test failed step
        success = log_ingest_step(
            trace_id, "test_failure", False, {"error": "Test error"}
        )
        if success:
            print("✓ Direct failure logging successful")
        else:
            print("✗ Direct failure logging failed")
