...
def score_v0(lead: Dict[str, Any]) -> Dict[str, Union[int, List[str]]]:
    ...
    if lead.get('created_at'):
        ...
        if recency_score > 20:
            reasons.append(f"Very recent lead (+{weighted_recency:.1f} pts: {days_old} days old)")
        elif recency_score > 15:
            reasons.append(f"Recent lead (+{weighted_recency:.1f} pts: {days_old} days old)")
        elif recency_score > 5:
            reasons.append(f"Moderately recent lead (+{weighted_recency:.1f} pts: {days_old} days old)")
        else:
            reasons.append(f"Older lead (+{weighted_recency:.1f} pts: {days_old} days old)")
    else:
    ...
    if matched_trade:
        reasons.append(f"High-value trade match: {matched_trade} (+{weighted_trade:.1f} pts)")
    elif lead.get('trade_tags'):
        reasons.append(f"Trade categories found but low-value (+{weighted_trade:.1f} pts)")
    else:
