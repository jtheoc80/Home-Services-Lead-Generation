...
class TestScoreV0:
    ...
    def test_score_v0_trade_scoring(self):
        ...
        assert (
            roofing_score > fence_score
        ), f"Roofing ({roofing_score}) should score higher than fence ({fence_score})"
        ...
        assert any(
            "No trade categories identified" in reason
            for reason in result_no_trade["reasons"]
        )
        assert (
            roofing_score > result_no_trade["score"]
        ), "Trade match should score higher than no trade"
    ...
    def test_score_v0_value_scoring(self):
        ...
        assert (
            result_high["score"] > result_medium["score"] > result_low["score"]
    ...
    def test_score_v0_property_age_scoring(self):
        ...
        assert (
            result_old["score"] > result_new["score"]
    ...
    def test_score_v0_owner_type_scoring(self):
        ...
        # Check reason strings
        assert any(
            "Individual owner" in reason for reason in result_individual["reasons"]
        )
    ...
    def test_score_v0_missing_data_handling(self):
        ...
        minimal_lead = {"lead_id": "test-minimal"}
    ...
    def test_score_v0_edge_cases(self):
        ...
        for value, expected_desc in boundary_tests:
            ...
            assert any(
                expected_desc in reason for reason in result["reasons"]
...
class TestScoringConsistency:
    ...
    def test_score_distribution(self):
        ...
        assert (
            low_scores > 0 or medium_scores > 0
        ), "Should have some leads scoring below 100"

        # Most quality leads will score high, but should have some variation
        assert perfect_scores < len(
            all_scores
        ), "Not all leads should score exactly 100"

        print(
            f"Score distribution: <80: {low_scores}, 80-99: {medium_scores}, 100: {perfect_scores}"
        )
    ...
    def test_high_score_leads_characteristics(self):
        ...
        for lead in high_score_leads:
            ...
            if days_old > 30:
                ...
                assert any(
                    tag in ["roofing", "kitchen", "bath", "foundation"]
                    for tag in lead.get("trade_tags", [])
                ) or (
    ...
    def test_low_score_leads_characteristics(self):
        ...
        for lead in low_score_leads:
            ...
            limiting_factors = [
                ...
                not lead.get("trade_tags")
                or lead["trade_tags"] == []
                or "fence" in lead.get("trade_tags", []),  # Poor trade match
                lead.get("year_built", 1900) > 2010,  # New property
                lead.get("owner_kind")
                in ["corporation", "llc"],  # Non-individual owner
            ]
