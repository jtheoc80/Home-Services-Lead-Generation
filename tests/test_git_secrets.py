...
class GitSecretsTest(unittest.TestCase):
    ...
    def test_supabase_service_role_key_detection(self):
        ...
        self.assertEqual(
            result.returncode, 1, "git-secrets should detect Supabase service role key"
        )
        self.assertIn(
            "sb-", result.stderr, "Error message should mention the detected pattern"
        )
    ...
    def test_jwt_token_detection(self):
        ...
        self.assertEqual(result.returncode, 1, "git-secrets should detect JWT tokens")
        self.assertIn(
            "ey", result.stderr, "Error message should mention the detected pattern"
        )
    ...
    def test_vercel_token_detection(self):
        ...
        self.assertEqual(
            result.returncode, 1, "git-secrets should detect Vercel tokens"
        )
    ...
    def test_railway_token_detection(self):
        ...
        self.assertEqual(
            result.returncode, 1, "git-secrets should detect Railway tokens"
        )
    ...
    def test_allowed_placeholder_values(self):
        ...
        self.assertEqual(
            result.returncode, 0, "git-secrets should allow placeholder values"
        )
...
"""
        ...
        self.assertEqual(result.returncode, 0, "git-secrets should allow clean files")
...
def demonstrate_failing_test():
    ...
    print(
        "This test creates a file with a fake Supabase key and shows how git-secrets blocks it."
    )
    ...
    try:
        ...
        with open(test_file, "w") as f:
            ...
            f.write(
                "SUPABASE_SERVICE_ROLE_KEY=sb-1234567890abcdef1234567890abcdef12345678\n"
            )
        ...
        result = subprocess.run(
            ["git", "secrets", "--scan", str(test_file)], capture_output=True, text=True
        )
