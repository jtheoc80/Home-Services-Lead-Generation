...
def demo_configuration():
    ...
    for source in config["sources"]:
        print(
            f"   • {source['id']:20} | {source['kind']:8} | {source.get('city', 'N/A'):10} | {source.get('cadence', 'N/A')}"
        )
...
def demo_connectors():
    ...
    print("✅ Socrata connector created for Dallas permits")
    ...
    print("✅ ArcGIS connector created for Arlington permits")
...
def demo_api_structure():
    ...
    try:
        ...
        for route in app.routes:
            if hasattr(route, "path") and any(
                keyword in route.path for keyword in ["demo/permits", "leads/scores"]
            ):
        ...
        for path, methods in tx_routes:
            methods_str = (
                ", ".join(methods) if isinstance(methods, (list, set)) else str(methods)
            )
...
def demo_frontend():
    ...
    if demo_page.exists():
        ...
        features = {
            ...
            "Responsive design": "grid" in content.lower()
            and "responsive" in content.lower(),
        }
        ...
        print("📊 API Calls: /api/demo/permits and /api/leads/scores")
...
def demo_workflow():
    ...
    if workflow_file.exists():
        ...
        print("🎛️ Manual trigger: GitHub Actions UI")
