-- Update upsert_leads_from_permits function to include zipcode mapping
-- This migration adds zip field mapping from permits.zipcode to leads.zip
...
BEGIN
  ...
  WITH upsert_results AS (
    ...
    SELECT 
      ...
      p.address,
      p.city,
      NULLIF(p.zipcode,'') as zip,  -- Map permits.zipcode to leads.zip
    ...
    ON CONFLICT (permit_id) DO UPDATE SET
      ...
      city = EXCLUDED.city,
      zip = EXCLUDED.zip,  -- Update zip field on conflict
...
'RPC function that upserts leads from permits data. Accepts optional p_days parameter to filter permits from the last N days. Uses public.permits.issued_date (not issue_date) for correct date handling. Maps permits.zipcode to leads.zip field. Returns counts of inserted, updated, and total processed records.';