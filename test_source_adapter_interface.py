...
def test_adapter_interface(adapter: SourceAdapter) -> None:
    ...
    assert hasattr(
        adapter, "name"
    ), f"{adapter.__class__.__name__} missing 'name' attribute"
    assert hasattr(
        adapter, "fetch"
    ), f"{adapter.__class__.__name__} missing 'fetch' method"
    assert hasattr(
        adapter, "parse"
    ), f"{adapter.__class__.__name__} missing 'parse' method"
    assert hasattr(
        adapter, "normalize"
    ), f"{adapter.__class__.__name__} missing 'normalize' method"
    ...
    assert (
        result == adapter.name
    ), f"Protocol check failed for {adapter.__class__.__name__}"
    ...
    socrata_adapter = SimpleSocrataAdapter(
        {
            ...
            "field_map": {"permit_number": "permit_number", "address": "address"},
        }
    )
    ...
    arcgis_adapter = ArcGISFeatureServiceAdapter(
        {
            ...
            "mappings": {"permit_number": "PERMITNUMBER", "address": "FULLADDRESS"},
        }
    )
    ...
    tpia_adapter = TPIAAdapter(
        {
            ...
            "mappings": {"permit_number": "Permit Number", "address": "Address"},
        }
    )
...
def main():
    ...
    adapters = [
        SimpleSocrataAdapter(
            {"name": "Dallas", "url": "test", "date_field": "issued_date"}
        ),
        ArcGISFeatureServiceAdapter(
            {"name": "Harris County", "url": "test", "date_field": "ISSUEDDATE"}
        ),
