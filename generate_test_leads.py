...
def generate_test_permits() -> List[PermitRecord]:
    ...
    for i in range(10):
        ...
        logger.info(
            f"Generated permit {i+1}: {permit.permit_id} for {permit.jurisdiction}"
        )
...
def push_to_supabase(permits: List[PermitRecord]) -> bool:
    ...
    if not os.getenv("SUPABASE_URL") or not os.getenv("SUPABASE_SERVICE_ROLE_KEY"):
        ...
        logger.warning(
            "Please set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY to enable Supabase integration."
        )
    ...
    try:
        ...
        for jurisdiction, jurisdiction_permits in jurisdiction_groups.items():
            ...
            logger.info(
                f"Pushing {len(jurisdiction_permits)} permits for {jurisdiction} to table {table_name}"
            )
            ...
            logger.info(
                f"Jurisdiction {jurisdiction}: {result['success']} success, {result['failed']} failed"
            )

        logger.info(
            f"Total Supabase upsert completed: {total_success} success, {total_failed} failed"
        )
...
def main():
    ...
    if success:
        ...
        logger.warning(
            "⚠️  Some issues occurred while pushing to Supabase (see logs above)"
        )
    ...
    logger.info(
        f"Supabase integration: {'✅ SUCCESS' if success else '⚠️  PARTIAL/FAILED'}"
    )
    ...
    for i, permit in enumerate(permits[:3]):  # Show first 3
        logger.info(
            f"  {i+1}. {permit.permit_id} - {permit.work_class} - ${permit.value:,} - {permit.jurisdiction}"
        )
