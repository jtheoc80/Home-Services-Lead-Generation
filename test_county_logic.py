...
def simulate_name_logic(
    work_description, permit_type, permit_number, permit_id, record_id
):
    """
...
def run_tests():
    ...
    county_tests = [
        ...
        (
            ...
            "test",
            "Dallas County",
            "Empty county - should infer from jurisdiction",
        ),
        (
            None,
            "tx-fort-bend",
            "test",
            "Fort Bend County",
            "NULL county - should infer from jurisdiction",
        ),
        (
            "",
            "",
            "tx-brazoria",
            "Brazoria County",
            "Empty county/jurisdiction - should infer from source",
        ),
        (
            "",
            ...
            "unknown-source",
            "Unknown",
            "No county info - should default to Unknown",
        ),
        (
            "  ",
            "tx-galveston",
            "test",
            "Galveston County",
            "Whitespace county - should infer from jurisdiction",
        ),
    ]

    print("County Logic Tests:")
    all_passed = True
    for i, (county, jurisdiction, source, expected, description) in enumerate(
        county_tests, 1
    ):
        ...
        print(
            f"     Input: county='{county}', jurisdiction='{jurisdiction}', source='{source}'"
        )
    ...
    name_tests = [
        ...
        (
            ...
            "uuid-2",
            "Commercial",
            "Empty description - use permit type",
        ),
        (
            "",
            "",
            ...
            "uuid-3",
            "Permit P-789",
            "No description/type - use permit number",
        ),
        (
            "",
            "",
    ...
    for i, (
        ...
        expected,
        description,
    ) in enumerate(name_tests, 1):
        result = simulate_name_logic(
            work_desc, permit_type, permit_number, permit_id, record_id
        )
        status = "✅ PASS" if result == expected else "❌ FAIL"
        if result != expected:
            all_passed = False
