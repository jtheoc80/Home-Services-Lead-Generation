...
def test_requirement_2_source_name():
    ...
    assert scraper.source_name == 'harris_issued_permits', f"Expected 'harris_issued_permits', got '{scraper.source_name}'"
...
def test_requirement_3_read_last_run():
    ...
    print("✅ Can read last_run for harris_issued_permits (returns None when Supabase unavailable)")
...
def test_requirement_5_issueddate_field():
    ...
    assert harris.source_config['date_field'] == 'ISSUEDDATE', f"Expected ISSUEDDATE, got {harris.source_config['date_field']}"
    
    # Create scraper and verify it uses the correct field
    scraper = adapter.create_scraper(harris)
    assert scraper.date_field == 'ISSUEDDATE', f"Expected ISSUEDDATE, got {scraper.date_field}"
...
def test_requirement_6_update_on_success():
    ...
    with patch('requests.get', return_value=mock_response):
        ...
        assert update_called[0][0] == 'harris_issued_permits', "Should update harris_issued_permits"
        assert isinstance(update_called[0][1], datetime), "Should pass datetime timestamp"
...
def test_requirement_7_query_format():
    ...
    with patch('requests.get', side_effect=mock_get):
        ...
        assert 'ISSUEDDATE > TIMESTAMP' in where_clause, f"Should query ISSUEDDATE, got: {where_clause}"
        assert '2025-01-15 10:30:00' in where_clause, f"Should include timestamp, got: {where_clause}"
    
    print("✅ Generates correct ArcGIS query with ISSUEDDATE > TIMESTAMP 'YYYY-MM-DD HH:MM:SS'")
