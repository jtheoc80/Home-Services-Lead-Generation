...
def test_work_type_normalization():
    ...
    assert (
        normalizer._normalize_work_type("Kitchen remodel", "Residential")
        == "residential"
    )
    assert (
        normalizer._normalize_work_type("Single family dwelling", None) == "residential"
    )
    assert (
        normalizer._normalize_work_type("Garage addition", "Building") == "residential"
    )

    # Test commercial patterns
    assert (
        normalizer._normalize_work_type("Office building", "Commercial") == "commercial"
    )
    assert (
        normalizer._normalize_work_type("Retail store renovation", None) == "commercial"
    )

    # Test multi-family
    assert normalizer._normalize_work_type("Apartment complex", None) == "multi_family"
    assert (
        normalizer._normalize_work_type("Condo renovation", "Multi-family")
        == "multi_family"
    )
...
def test_coordinate_extraction():
    ...
    raw_record = {"latitude": "32.7767", "longitude": "-96.7970"}  # Dallas coordinates
