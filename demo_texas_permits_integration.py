...
def test_austin_api():
    ...
    try:
        ...
        print("✓ Austin API accessible")

        print("📝 Testing sample query...")
        sample_data = fetch(domain="data.austintexas.gov", dataset="3syk-w9eu", limit=5)
        print(f"✓ Retrieved {len(sample_data)} sample records")
...
def test_arlington_api():
    ...
    try:
        ...
        print("✓ Arlington API accessible")
...
def test_data_normalization(dallas_data, austin_data, arlington_data):
    ...
    for i, record in enumerate(dallas_data[:2]):
        ...
        if normalized:
            print(
                f"  ✓ Record {i+1}: {normalized['permit_id']} - {normalized['work_type']}"
            )
