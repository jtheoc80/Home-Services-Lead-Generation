    ...
    try:
        ...
        with IngestTracer() as tracer:
            ...
            success = tracer.log(
                "parse",
                True,
                {"permits_parsed": 40, "parse_errors": 2, "parse_time_ms": 150},
            )
            ...
            success = tracer.log(
                "db_insert",
                True,
                {"final_count": 40, "total_time_ms": 900, "status": "completed"},
            )
        ...
        if response.status_code == 200:
            ...
            if data.get("logs"):
                ...
                for i, log in enumerate(data["logs"], 1):
                    print(
                        f"   {i}. {log.get('stage', 'unknown')} - {'✓' if log.get('ok') else '✗'}"
                    )
                    if log.get("details"):
...
def main():
    ...
    if trace_id:
        ...
        print("   With header: X-Debug-Key: test-debug-key-123")
