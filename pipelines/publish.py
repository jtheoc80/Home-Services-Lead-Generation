...
class LeadPublisher:
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if not exists:
                        logger.error("gold.lead_scores table does not exist. Run 'make db-migrate' first.")
    ...
    def get_new_or_updated_permits(self, since: Optional[datetime] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        if since is None:
            since = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)  # Today
    ...
    def convert_permit_to_lead(self, permit: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lead = {
            ...
            # Scoring fields
            'created_at': permit.get('issued_at') or permit.get('applied_at') or datetime.now(),
    ...
    def _infer_owner_kind(self, permit: Dict[str, Any]) -> str:
        ...
        if any(suffix in contractor_name for suffix in ['LLC', 'CORP', 'INC', 'COMPANY']):
            return 'llc'
        elif contractor_name and any(indicator in contractor_name for indicator in ['OWNER', 'SELF']):
            return 'individual'
        else:
            return 'unknown'  # Default when we can't determine
    
    def compute_and_publish_scores(self, permits: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
                ...
                with conn.cursor() as cur:
                    for permit in permits:
                        try:
                            ...
                            logger.error(f"Failed to score permit {permit.get('permit_id')}: {e}")
                            errors += 1
    ...
    def _upsert_lead_score(self, cursor, lead_id: str, version: str, score: int, reasons: List[str]):
        """
...
def publish_from_normalization_results(normalization_results: Dict[str, Any]) -> Dict[str, Any]:
    """
