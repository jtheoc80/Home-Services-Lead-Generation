        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if not exists:
                        logger.error(
                            "gold.permits table does not exist. Run 'make db-migrate' first."
                        )
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    for source_id, raw_records in source_data.items():
                        ...
                        logger.info(
                            f"Normalizing {len(raw_records)} records from {source_id}"
                        )
                        ...
                        for raw_record in raw_records:
                            try:
                                ...
                                if validation_errors:
                                    logger.warning(
                                        f"Validation errors for {normalized.get('permit_id')}: {validation_errors}"
                                    )
                        ...
                        source_results[source_id] = {
                            ...
                            "upserted": len(
                                normalized_records
                            ),  # Assuming all go through
                        }

                    # Commit all changes
                    conn.commit()
                    logger.info(
                        f"Normalization complete: {total_processed} processed, {total_upserted} upserted, {total_errors} errors"
                    )
    ...
    def _get_source_metadata(self, source_id: str) -> Dict[str, Any]:
        ...
        if "dallas" in source_id.lower():
            source_metadata.update(
                {"jurisdiction": "Dallas", "city": "Dallas", "county": "Dallas"}
            )
        elif "austin" in source_id.lower():
            source_metadata.update(
                {"jurisdiction": "Austin", "city": "Austin", "county": "Travis"}
            )
        elif "arlington" in source_id.lower():
            source_metadata.update(
                {"jurisdiction": "Arlington", "city": "Arlington", "county": "Tarrant"}
            )
        elif "harris" in source_id.lower():
            source_metadata.update(
                {"jurisdiction": "Harris County", "city": "Houston", "county": "Harris"}
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Normalize permit data to gold.permits"
    )
    parser.add_argument(
