        ...
        if kind == "socrata":
            ...
            for updated_field in updated_fields:
                try:
                    ...
                    logger.warning(
                        f"Failed to load {source_id} with field {updated_field}: {e}"
                    )
                    continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load raw permit data from Texas sources"
    )
    ...
    for source in sources:
        if source["kind"] == "tpia":
            logger.info(
                f"Skipping TPIA source {source['id']} - requires manual processing"
            )
            continue
    ...
    logger.info(
        f"Load complete: {successful} successful, {failed} failed, {total_records} total records"
    )
...
try:
    ...
    def create_connector(source_config):
        logger.warning(
            f"Connector creation not available for {source_config.get('id')}"
        )
        return None
...
class RawDataLoader:
    ...
    def _load_sources_config(self) -> Dict[str, Any]:
        ...
        try:
            with open(self.sources_config_path, "r") as f:
                config = yaml.safe_load(f)
            logger.info(
                f"Loaded sources config with {len(config.get('tier_1_sources', []))} Tier-1 sources"
            )
            return config
        except Exception as e:
            logger.error(f"Failed to load sources config: {e}")
    ...
    def _update_ingest_state(
        ...
        try:
            ...
            """,
                {"source_id": source_id, **update_values},
            )
    ...
    def _store_raw_records(
        self, source_id: str, source_type: str, records: List[Dict[str, Any]]
    ) -> int:
    ...
    def ingest_source(
        self, source_config: Dict[str, Any], full_refresh: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not connector:
                if source_kind == "tpia":
                    logger.info(
                        f"Skipping TPIA source {source_id} - requires manual processing"
                    )
                    return {
                        "status": "skipped",
                        "reason": "TPIA requires manual processing",
                    }
                else:
            ...
            if not full_refresh:
                ...
                if last_ingest:
                    ...
                    logger.info(
                        f"First ingest - fetching last 30 days since {updated_since}"
                    )
    ...
    def run_tier1_ingests(self, full_refresh: bool = False) -> Dict[str, Any]:
        ...
        failed = sum(
            ...
            if r.get("status") in ["error", "critical_error"]
        )
        skipped = sum(1 for r in results.values() if r.get("status") == "skipped")

        logger.info(
            f"Tier-1 ingests complete: {successful} successful, {failed} failed, {skipped} skipped"
        )
    ...
    def run_tier2_ingests(self, full_refresh: bool = False) -> Dict[str, Any]:
        ...
        logger.info(
            f"Tier-2 ingests complete: {successful} successful, {failed} failed, {skipped} skipped"
        )
            ...
            try:
                ...
                for source in sources:
                    ...
                    state_manager.update_state(
                        source["id"], since_date, "manual_override"
                    )
                logger.info(f"Loading data since {args.since}")
            except ValueError as e:
                logger.error(f"Invalid since date format: {e}")
