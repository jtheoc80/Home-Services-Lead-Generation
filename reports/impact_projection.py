...
class ImpactProjectionReporter:
    ...
    def generate_impact_report(self, 
        ...
        report = {
            ...
            "summary": {
                "regions_with_increased_risk": len(comparison_data[comparison_data['change_pct'] > 0]),
                "regions_with_decreased_risk": len(comparison_data[comparison_data['change_pct'] < 0]),
                "avg_change_pct": float(comparison_data['change_pct'].mean()),
                "max_increase": {
                    "region": comparison_data.loc[comparison_data['change_pct'].idxmax(), 'region_name'],
                    "current_risk": float(comparison_data.loc[comparison_data['change_pct'].idxmax(), 'current_p_surge']),
                    "prior_risk": float(comparison_data.loc[comparison_data['change_pct'].idxmax(), 'prior_p_surge']),
                    "change_pct": float(comparison_data.loc[comparison_data['change_pct'].idxmax(), 'change_pct'])
                },
                "max_decrease": {
                    "region": comparison_data.loc[comparison_data['change_pct'].idxmin(), 'region_name'],
                    "current_risk": float(comparison_data.loc[comparison_data['change_pct'].idxmin(), 'current_p_surge']),
                    "prior_risk": float(comparison_data.loc[comparison_data['change_pct'].idxmin(), 'prior_p_surge']),
                    "change_pct": float(comparison_data.loc[comparison_data['change_pct'].idxmin(), 'change_pct'])
                }
            },
            "narrative": narrative,
    ...
    def _get_forecast_data(self, regions: Optional[List[str]], target_date: date) -> pd.DataFrame:
    ...
    def _merge_and_calculate_changes(self, current_data: pd.DataFrame, prior_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate changes
        merged['change_pct'] = ((merged['current_p_surge'] - merged['prior_p_surge']) / merged['prior_p_surge']) * 100
        merged['change_abs'] = merged['current_p_surge'] - merged['prior_p_surge']
        
        # Risk levels
        merged['current_risk_level'] = merged['current_p_surge'].apply(self._get_risk_level)
        merged['prior_risk_level'] = merged['prior_p_surge'].apply(self._get_risk_level)
        
        # Risk direction
        merged['risk_direction'] = np.where(
            merged['change_pct'] > 10, 'Significantly Increased',
            np.where(merged['change_pct'] > 0, 'Increased',
                    np.where(merged['change_pct'] < -10, 'Significantly Decreased', 'Decreased'))
        )
    ...
    def _generate_narrative(self, comparison_data: pd.DataFrame) -> Dict[str, str]:
        ...
        # Regional highlights
        high_risk_regions = comparison_data[comparison_data['current_risk_level'] == 'High']
    ...
    def _create_visualizations(self, comparison_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            risk_transitions = comparison_data.groupby(['prior_risk_level', 'current_risk_level']).size().reset_index(name='count')
            ...
            if not risk_transitions.empty:
                fig_transition = px.bar(
                    ...
                    labels={'count': 'Number of Regions', 'prior_risk_level': 'Prior Risk Level'}
                )
            ...
            summary_table = comparison_data[[
                'region_name', 'current_p_surge', 'prior_p_surge', 'change_pct', 
                'current_risk_level', 'risk_direction'
            ]].round(3)
    ...
    def _save_report(self, report: Dict[str, Any], output_format: str, target_date: date):
        """Save report to file"""
        try:
    ...
    def _save_html_report(self, report: Dict[str, Any], filepath: Path):
        ...
        for key, narrative in report['narrative'].items():
            html_content += f"<p><strong>{key.replace('_', ' ').title()}:</strong> {narrative}</p>"
    ...
    def generate_weekly_report(self, target_date: Optional[date] = None) -> Dict[str, Any]:
        """
...
def generate_example_report() -> Dict[str, Any]:
    ...
    # Calculate changes
    sample_data['change_pct'] = ((sample_data['current_p_surge'] - sample_data['prior_p_surge']) / sample_data['prior_p_surge']) * 100
    sample_data['change_abs'] = sample_data['current_p_surge'] - sample_data['prior_p_surge']
    
    reporter = ImpactProjectionReporter()
    sample_data['current_risk_level'] = sample_data['current_p_surge'].apply(reporter._get_risk_level)
    sample_data['prior_risk_level'] = sample_data['prior_p_surge'].apply(reporter._get_risk_level)
    
    sample_data['risk_direction'] = np.where(
        sample_data['change_pct'] > 10, 'Significantly Increased',
        np.where(sample_data['change_pct'] > 0, 'Increased',
                np.where(sample_data['change_pct'] < -10, 'Significantly Decreased', 'Decreased'))
    )
    ...
    return {
        ...
        "summary": {
            "regions_with_increased_risk": len(sample_data[sample_data['change_pct'] > 0]),
            "regions_with_decreased_risk": len(sample_data[sample_data['change_pct'] < 0]),
