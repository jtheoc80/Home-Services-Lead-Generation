...
def run_source_unified(source_cfg: Dict[str, Any], since_days: int, limit: int = 5000) -> None:
    """
        ...
        for raw_chunk in raw_data_chunks:
            for parsed_record in adapter.parse(raw_chunk):
                ...
                if is_residential:
                    ...
                    # Show sample record
                    if count_saved <= 3:
                        print(f"[SAMPLE] {normalized_record.get('permit_number')} - {normalized_record.get('address')}")
    ...
    print(f"[RESULT] {adapter.name}: {count_in} total, {count_residential} residential, {count_saved} saved")
...
def is_residential_permit(record: Dict[str, Any]) -> bool:
    ...
    RESIDENTIAL_KEYWORDS = [
        "single family", "duplex", "townhouse", "condo", "residential",
        "house", "home", "dwelling", "apartment", "kitchen", "bathroom",
        "bedroom", "garage", "fence", "pool", "deck", "patio"
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process permit data using unified SourceAdapter interface')
    parser.add_argument('--config', type=Path, default='config/sources.yaml',
                       help='Configuration file path')
    parser.add_argument('--days', type=int, default=7,
                       help='Number of days back to fetch data')
    parser.add_argument('--limit', type=int, default=5000,
                       help='Maximum records to process per source')
    parser.add_argument('--source', type=str,
                       help='Process only specific source by name')
