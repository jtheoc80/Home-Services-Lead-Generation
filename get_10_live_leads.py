...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Get 10 live permit leads and push through Supabase"
    )
    parser.add_argument(
        "--with-env",
        action="store_true",
    ...
    logger.info(
        f"  SUPABASE_URL: {'✅ Set' if os.getenv('SUPABASE_URL') else '❌ Not set'}"
    )
    logger.info(
    ...
    logger.info("=" * 70)
    logger.info(
        "✅ Task completed: Get 10 live permit leads from counties/cities and push through Supabase"
    )
    logger.info("")
    logger.info("What was accomplished:")
    logger.info("  📋 Generated/scraped 10 permit leads from configured counties:")
    logger.info(
        "     - Harris County, Fort Bend County, Brazoria County, Galveston County"
    )
