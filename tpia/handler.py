...
class TPIAHandler:
    ...
    def __init__(
        self,
        requests_dir: str = "tpia/requests",
        deliveries_dir: str = "tpia/deliveries",
    ):
        """
    ...
    def generate_houston_request(
        self, start_date: datetime, end_date: datetime, requestor_info: Dict[str, str]
    ) -> str:
    ...
    def process_delivered_csv(
        self, csv_path: str, source_id: str = "tx-houston-tpia", archive: bool = True
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    generate_parser = subparsers.add_parser(
        "generate-request", help="Generate TPIA request letter"
    )
    ...
    generate_parser.add_argument(
        "--requestor-name", required=True, help="Requestor name"
    )
    generate_parser.add_argument(
        "--organization", required=True, help="Organization name"
    )
    ...
    process_parser = subparsers.add_parser(
        "process-delivery", help="Process delivered CSV file"
    )
    process_parser.add_argument("csv_file", help="Path to delivered CSV file")
    process_parser.add_argument(
        "--source-id", default="tx-houston-tpia", help="Source identifier"
    )
    process_parser.add_argument(
        "--no-archive", action="store_true", help="Do not archive original file"
    )
    ...
    try:
        if args.command == "generate-request":
            ...
            request_path = handler.generate_houston_request(
                start_date, end_date, requestor_info
            )
            print(f"Generated TPIA request: {request_path}")

        elif args.command == "process-delivery":
            result = handler.process_delivered_csv(
                args.csv_file, args.source_id, archive=not args.no_archive
            )
