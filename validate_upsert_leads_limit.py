...
def validate_function_call():
    ...
    if actual_param_names == expected_param_names:
        ...
        print(f"❌ Parameter mismatch. Expected: {expected_param_names}, Got: {actual_param_names}")
        return False
...
def validate_sql_function_signature():
    ...
    try:
        with open('supabase/migrations/20250121000000_add_upsert_leads_from_permits_limit.sql', 'r') as f:
            sql_content = f.read()
    ...
    for check_name, check_pattern in checks:
        if check_pattern in sql_content:
            if check_name == "No incorrect date column":
                ...
                if column_matches:
                    print(f"❌ {check_name}: Found column usage of {check_pattern}: {column_matches}")
                    return False
                else:
                    print(f"✅ {check_name}: {check_pattern} not used as column reference (good)")
            else:
