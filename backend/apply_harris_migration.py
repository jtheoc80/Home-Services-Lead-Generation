...
def get_database_url():
    ...
    return os.environ.get(
        "DATABASE_URL", "postgresql://postgres:password@localhost:5432/leadledger"
    )
...
def create_sample_harris_data(conn):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "DELETE FROM permits_raw_harris WHERE event_id LIKE 'HARRIS_DEMO_%'"
            )
...
def test_normalization_function(conn):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Check initial state
            cur.execute(
                "SELECT COUNT(*) as count FROM permits_raw_harris WHERE event_id LIKE 'HARRIS_DEMO_%'"
            )
            raw_count = cur.fetchone()["count"]
            print(f"Found {raw_count} raw Harris County permits")

            cur.execute(
                "SELECT COUNT(*) as count FROM leads WHERE source_ref LIKE 'HARRIS_DEMO_%'"
            )
            ...
            for permit in normalized_permits:
                print(
                    f"   {permit['source_ref']}: {permit['permit_id']} - {permit['address']}"
                )
                print(
                    f"      Category: {permit['category']}, Value: ${permit['value']:,.2f}"
                )
                print(f"      Trade Tags: {permit['trade_tags']}")
                print(
                    f"      County: {permit['county']}, Type: {permit['permit_type']}"
                )
                print()
...
def main():
    ...
    try:
        ...
        migration_file = (
            ...
            / "migrations"
            / "005_harris_permits_normalization.sql"
        )
