...
def demo_export_control():
    ...
    request = controller.create_export_request(
        ExportType.LEADS, "demo_user", {"format": "csv", "date_range": "30_days"}
    )
...
def demo_notifications():
    ...
    print(
        f"SendGrid API Key: {'✅ Set' if service.config.sendgrid_api_key else '❌ Not set'}"
    )
    ...
    sms_result = service.sms.send_sms(
        to_number="+1234567890", message="Test SMS notification"
    )
...
def demo_pricing_config():
    ...
    for option in recommendation["all_options"]:
        print(
            f"  {option['tier']:10} - ${option['total_cost']:6} "
