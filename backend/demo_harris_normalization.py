...
class HarrisPermitNormalizer:
    ...
    def normalize_permits_harris(self) -> Dict[str, int]:
        ...
        for permit in self.permits_raw_harris:
            ...
            if event_id not in event_ids or permit['created_at'] > event_ids[event_id]['created_at']:
    ...
    def _normalize_permit_data(self, raw_permit: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if ('residential' in permit_type or 
            any(keyword in work_description for keyword in ['single family', 'duplex', 'residential', 'house', 'home'])):
            category = 'residential'
        elif ('commercial' in permit_type or 
              any(keyword in work_description for keyword in ['commercial', 'office', 'retail', 'industrial'])):
            category = 'commercial'
        else:
            category = 'other'
        ...
        if any(keyword in desc_lower for keyword in ['electric', 'electrical', 'wiring']):
            trade_tags.append('electrical')
        if any(keyword in desc_lower for keyword in ['hvac', 'heating', 'cooling', 'air']):
        ...
        return {
            ...
            'applicant': raw_permit.get('applicant_name') or raw_permit.get('contractor_name'),
...
def demo_harris_normalization():
    ...
    print(f"   📊 Second normalization - Processed: {result2['processed_count']}, New: {result2['new_count']}, Updated: {result2['updated_count']}")
    
    # Check updated record
    updated_lead = normalizer.get_leads_by_source_ref('HARRIS_DEMO_001')
    print(f"   ✅ Updated permit status: {updated_lead['status']}, value: ${updated_lead['value']:,.2f}")
    ...
    if harris_leads:
        ...
        print(f"   work_description → description: {sample_lead['description'][:50]}...")
