...
def demo_export_security():
    ...
    response = requests.post(f"{API_BASE}/api/export/data", json=export_request, headers=headers_user)
    ...
    response = requests.post(f"{API_BASE}/api/export/data", json=override_request, headers=headers_user)
    print(f"   Status: {response.status_code}")
    print(f"   Response: {response.json()}")
    
    print("\n3. ❌ Admin user without override (should get 403)")
    response = requests.post(f"{API_BASE}/api/export/data", json=export_request, headers=headers_admin)
    print(f"   Status: {response.status_code}")
    print(f"   Response: {response.json()}")
    
    print("\n4. ✅ Admin user with override (should get 200)")
    response = requests.post(f"{API_BASE}/api/export/data", json=override_request, headers=headers_admin)
    print(f"   Status: {response.status_code}")
...
if __name__ == "__main__":
    try:
        ...
        print("ALLOW_EXPORTS=false SUPABASE_JWT_SECRET=test_secret uvicorn main:app --port 8000")