...
try:
    ...
    print(
        "Error: The 'python-dotenv' package is required to run this script. Please install it with 'pip install python-dotenv'.",
        file=sys.stderr,
    )
    sys.exit(1)
        ...
        with conn.cursor() as cursor:
            ...
            for statement in statements:
                ...
                try:
                    ...
                    if statement.upper().startswith("CREATE TYPE"):
                        ...
                        if match:
                            ...
                            if "." in qualified_type_name:
                                schema_name, type_name = qualified_type_name.split(
                                    ".", 1
                                )
                                ...
                                if ns_row:
                                    ...
                                    if cursor.fetchone():
                                        logger.info(
                                            f"Type {qualified_type_name} already exists, skipping..."
                                        )
                                        continue
                            else:
                    ...
                    # we can safely ignore it
                    if e.pgcode == "42710" and statement.upper().startswith(
                        "CREATE TYPE"
                    ):
