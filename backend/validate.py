...
def test_database_schema():
    ...
    try:
        ...
        feedback_schema_file = (
            Path(__file__).parent / "app" / "migrations" / "001_feedback_tables.sql"
        )
        ...
        for element in feedback_required_elements:
            assert (
                element in feedback_content
            ), f"Missing feedback schema element: {element}"

        print("✓ Feedback tables schema is valid")

        # Test notifications table migration
        notifications_schema_file = (
            Path(__file__).parent / "app" / "migrations" / "002_notifications_table.sql"
        )
        assert (
            notifications_schema_file.exists()
        ), "Missing notifications migration file"
        ...
        for element in notifications_required_elements:
            assert (
                element in notifications_content
            ), f"Missing notifications schema element: {element}"
        ...
        assert (
            "CREATE TABLE IF NOT EXISTS notifications" in models_content
        ), "Notifications table missing from models.sql"
...
def main():
    ...
    if passed == total:
        ...
        print("\nNext steps:")
        print(
            '1. Run database migration: psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -b -e -f backend/app/models.sql'
        )
