...
class TestHarrisPermitNormalization:
    ...
    def db_connection(self):
        ...
        class MockCursor:
                ...
                if "INSERT INTO permits_raw_harris" in query:
                    ...
                    for permit in raw_permits:
                        ...
                        if (
                            ...
                            or permit["id"] > event_ids[event_id]["id"]
                        ):
                    ...
                    for event_id, raw_permit in event_ids.items():
                        ...
                        normalized = {
                            "id": (
                                len(self.data["leads"]) + 1
                                if not existing_lead
                                else existing_lead["id"]
                            ),
                            ...
                            "category": (
                                "residential"
                                if "residential"
                                in raw_permit.get("permit_type", "").lower()
                                else "commercial"
                            ),
            ...
            def fetchone(self):
                return (
                    self.result[0] if hasattr(self, "result") and self.result else None
                )
    ...
    def test_normalize_permits_harris_function(
        self, test_harris_permits, db_connection
    ):
        ...
        residential_permit = next(
            p for p in normalized_permits if p["source_ref"] == "HARRIS_2024_001"
        )
        ...
        renovation_permit = next(
            p for p in normalized_permits if p["source_ref"] == "HARRIS_2024_002"
        )
        assert renovation_permit["category"] == "residential"
        assert renovation_permit["value"] == 85000.0

        # Check third permit (commercial)
        commercial_permit = next(
            p for p in normalized_permits if p["source_ref"] == "HARRIS_2024_003"
        )
    ...
    def test_trade_tags_extraction(self, db_connection):
        ...
        cur.execute(
            "SELECT * FROM leads WHERE source_ref IN ('TAG_TEST_001', 'TAG_TEST_002')"
        )
        permits = cur.fetchall()
