...
class TestMetricsMiddlewareIntegration(unittest.TestCase):
            ...
            async def metrics_endpoint(auth: bool = Depends(verify_test_auth)):
                try:
                    ...
                    return Response(
                        ...
                        media_type=self.get_metrics_content_type()
                    )
                except Exception as e:
    ...
    def test_middleware_tracks_successful_requests(self):
        ...
        duration_lines = [line for line in metrics_output.split('\n') 
                         if 'http_request_duration_seconds' in line and not line.startswith('#')]
        
        # Should have some metrics
        self.assertGreater(len(metric_lines), 0, f"No request metrics found. Output:\n{metrics_output}")
        self.assertGreater(len(duration_lines), 0, f"No duration metrics found. Output:\n{metrics_output}")
        
        # Check for specific request patterns in the metrics
        found_get_200 = any('method="GET"' in line and 'status="200"' in line for line in metric_lines)
        found_post_200 = any('method="POST"' in line and 'status="200"' in line for line in metric_lines)
        
        self.assertTrue(found_get_200, f"GET 200 not found in metrics: {metric_lines}")
        self.assertTrue(found_post_200, f"POST 200 not found in metrics: {metric_lines}")
    ...
    def test_path_normalization(self):
        ...
        self.assertTrue(found_test_path, f"Path /test not found in metrics: {metric_lines}")
