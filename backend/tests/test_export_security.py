...
class TestExportSecurity:
    ...
    def test_export_blocked_when_allow_exports_false_non_admin(self, client, setup_env):
        ...
        response = client.post(
            "/api/export/data",
            json={"export_type": "leads", "format": "csv"},
    ...
    def test_export_blocked_when_allow_exports_false_admin_no_override(
        self, client, setup_env
    ):
    ...
    def test_admin_override_allows_export_when_allow_exports_false(
        self, client, setup_env
    ):
        ...
        response = client.post(
            "/api/export/data",
            json={"export_type": "leads", "format": "csv", "admin_override": True},
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    def test_non_admin_cannot_use_override(self, client, setup_env):
        ...
        assert response.status_code == 403
        assert (
            "Admin privileges required for export override" in response.json()["detail"]
        )
    ...
    def test_invalid_export_type_returns_400(self, client, setup_env):
        ...
        response = client.post(
            "/api/export/data",
            json={"export_type": "invalid_type", "format": "csv"},
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    def test_export_status_shows_admin_capabilities(self, client, setup_env):
        ...
        response = client.get(
            "/api/export/status", headers={"Authorization": f"Bearer {admin_token}"}
        )
        ...
        response = client.get(
            "/api/export/status", headers={"Authorization": f"Bearer {user_token}"}
        )
    ...
    def test_unauthorized_request_returns_403(self, client, setup_env):
        ...
        response = client.post(
            "/api/export/data", json={"export_type": "leads", "format": "csv"}
        )
...
def create_jwt_token(user_id: str, email: str, role: str = "user") -> str:
    ...
    def test_blocked_export_is_audited(self, mock_logger, setup_env):
        ...
        allowed, reason = controller.is_export_allowed(
            ExportType.LEADS, "user@test.com", {"test": "parameter"}
        )
    ...
    def test_successful_export_is_audited(self, mock_logger, setup_env):
        ...
        audit_logged = any(
            "AUDIT: Export successful" in str(call)
