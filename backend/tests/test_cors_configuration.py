...
class TestCORSConfiguration:
    ...
    def test_cors_origins_configuration(self):
        ...
        if hasattr(cors_middleware, 'kwargs'):
            ...
            for expected_origin in expected_origins:
                assert expected_origin in origins, f"Expected origin {expected_origin} should be in CORS configuration"
        else:
    ...
    def test_cors_other_settings(self):
        ...
        if hasattr(cors_middleware, 'kwargs'):
            ...
            assert kwargs.get('allow_credentials') is True, "CORS should allow credentials"
...
def test_cors_configuration_in_source():
    ...
    assert "http://localhost:3000" in content, "localhost:3000 should be in CORS origins"
    assert "https://<your-staging-domain>" in content, "staging domain placeholder should be in CORS origins"
    assert "https://<your-prod-domain>" in content, "prod domain placeholder should be in CORS origins"
    
    # Check that the old 127.0.0.1 origin is not present anymore
    assert "http://127.0.0.1:3000" not in content, "127.0.0.1:3000 should not be in CORS origins"
