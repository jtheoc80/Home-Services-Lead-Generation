...
class TestPermitIntegration:
    ...
    def test_seed_permits_in_database(
        self, test_permits, mock_supabase_client, mock_db_transaction
    ):
        ...
        def side_effect_insert():
            ...
            insert_call_args = (
                mock_table.insert.call_args[0][0] if mock_table.insert.call_args else {}
            )
        ...
        assert first_permit["trade_tags"] == [
            "plumbing",
            "electrical",
            "general_contractor",
        ]
    ...
    def test_react_component_snapshot(
        self, test_permits, mock_supabase_client, mock_db_transaction
    ):
        ...
        for element in expected_elements:
            assert (
                element in component_html
            ), f"Expected element '{element}' not found in component snapshot"
    ...
    def test_full_integration_workflow(
        ...
        try:
            ...
            assert (
                final_count == 0
            ), f"Expected 0 permits after rollback, found {final_count}"
