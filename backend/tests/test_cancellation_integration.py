...
def test_cancellation_feedback_service():
    ...
    with patch("psycopg2.connect") as mock_connect:
        ...
        assert (
            adjustments["city_of_houston"]["score_adjustment"] < 0
        )  # Should be negative
        ...
        adjustment = service.calculate_personalized_adjustments(
            "test-account", lead_features
        )
...
def test_ml_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with patch("sys.path", [temp_dir] + sys.path):
            ...
            mock_model_data["model"].predict_proba.return_value = [
                [0.3, 0.7],
                [0.8, 0.2],
            ]
            mock_model_data["model"].predict.return_value = [True, False]
            mock_model_data["scaler"].transform.return_value = [
                [1, 2, 3, 0.1, 0, 0.15],
                [2, 3, 4, 0.2, 1, 0.05],
            ]
