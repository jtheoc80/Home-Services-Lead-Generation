...
async def use_credits(user_id: str, quantity: int) -> bool:
    ...
    try:
        ...
        if current_balance < quantity:
            logger.warning(
                f"Insufficient credits for user {user_id}: {current_balance} < {quantity}"
            )
            return False
        ...
        logger.info(
            f"Used {quantity} credits for user {user_id}, new balance: {new_balance}"
        )
        return True
...
async def create_customer(user: AuthUser = Depends(auth_user)) -> CustomerResponse:
    ...
    try:
        ...
        result = (
            client.table("billing_customers")
            .select("*")
            .eq("user_id", user.account_id)
            .execute()
        )
        ...
        stripe_customer = stripe.Customer.create(
            email=user.email, metadata={"user_id": user.account_id}
        )
        ...
        logger.info(
            f"Created Stripe customer {stripe_customer.id} for user {user.account_id}"
        )

        return CustomerResponse(customer_id=stripe_customer.id, email=user.email)
...
async def create_checkout_session_subscription(
    ...
    try:
        ...
        session_params = {
            ...
            "mode": "subscription",
            "success_url": stripe_client.success_url
        ...
        logger.info(
            f"Created subscription checkout session {session.id} for user {user.account_id}"
        )
        ...
        logger.error(
            f"Failed to create subscription checkout for user {user.account_id}: {e}"
        )
...
async def create_checkout_session_credits(
    ...
    try:
        ...
        if not stripe_client.price_lead_credit_pack:
            raise HTTPException(
                status_code=400, detail="Lead credit pack price not configured"
            )
        ...
        session_params = {
            ...
            + "?session_id={CHECKOUT_SESSION_ID}",
            "cancel_url": stripe_client.cancel_url,
            "metadata": {"user_id": user.account_id, "type": "credit_pack"},
        }
        ...
        logger.info(
            f"Created credit pack checkout session {session.id} for user {user.account_id}"
        )
        ...
        logger.error(
            f"Failed to create credit checkout for user {user.account_id}: {e}"
        )
        raise HTTPException(status_code=500, detail="Failed to create checkout session")
    ...
    try:
        ...
        session = stripe.billing_portal.Session.create(
            ...
            return_url=os.getenv(
                "BILLING_SUCCESS_URL", "https://localhost:3000/billing"
            ),
        )
