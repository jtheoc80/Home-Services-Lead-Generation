...
class SubscriptionManager:
    ...
    def is_quiet_hours(self, now: Optional[datetime] = None) -> bool:
        ...
        if self.quiet_hours.start_hour > self.quiet_hours.end_hour:
            # e.g., 22:00 to 08:00
            return (
                current_hour >= self.quiet_hours.start_hour
                or current_hour < self.quiet_hours.end_hour
            )
        else:
            # e.g., 01:00 to 06:00
            return (
                current_hour >= self.quiet_hours.start_hour
                and current_hour < self.quiet_hours.end_hour
            )

    def schedule_notification_respecting_quiet_hours(
        self, notification_func, *args, **kwargs
    ) -> bool:
        """
        ...
        if self.is_quiet_hours():
            logger.info(
                "Currently in quiet hours. Notification would be deferred for later sending."
            )
    ...
    def cancel_subscription(
        self, subscription: SubscriptionInfo, request: CancellationRequest
    ) -> Dict[str, Any]:
        """
    ...
    def _cancel_trial_subscription(
        self,
    ...
    def _cancel_paid_subscription(
        ...
        if grace_reminder_date > now:
            ...
            logger.info(
                f"Grace reminder scheduled for {grace_reminder_date} for user {subscription.user_id}"
            )
    ...
    def reactivate_subscription(self, subscription: SubscriptionInfo) -> Dict[str, Any]:
        """
        ...
        notification_sent = self.schedule_notification_respecting_quiet_hours(
            self._send_reactivation_notification, subscription.user_id
        )
    ...
    def send_access_ended(self, subscription: SubscriptionInfo) -> bool:
        ...
        """
        return self.schedule_notification_respecting_quiet_hours(
            self._send_access_ended_notification, subscription.user_id
        )

    def _send_cancel_confirm_notification(
        self,
        user_id: str,
        ...
        if cancellation_type == CancellationType.TRIAL:
            ...
            message = (
                "Your LeadLedgerPro trial has been cancelled and access has ended immediately. "
                "We're sorry to see you go! If you change your mind, you can sign up again anytime."
            )
        else:
            grace_date_str = (
                grace_end_date.strftime("%B %d, %Y")
                if grace_end_date
                else "your billing period end"
            )
            subject = "Subscription Cancellation Confirmed - LeadLedgerPro"
            message = (
                f"Your LeadLedgerPro subscription has been cancelled. "
    ...
    def _send_grace_reminder_notification(
        self, user_id: str, grace_end_date: datetime
    ) -> bool:
        """Send grace period reminder notification."""
        ...
        message = (
            f"This is a reminder that your LeadLedgerPro access will end on {grace_date_str}. "
    ...
    def _send_access_ended_notification(self, user_id: str) -> bool:
        ...
        message = (
            "Your LeadLedgerPro subscription has ended and access has been terminated. "
    ...
    def _send_reactivation_notification(self, user_id: str) -> bool:
        ...
        message = (
            "Great news! Your LeadLedgerPro subscription has been reactivated. "
