    ...
    def sync_regions_to_db(self) -> None:
        ...
        try:
            ...
            for region in regions.values():
                ...
                if region.parent:
                    cur.execute(
                        "SELECT id FROM regions WHERE slug = %s", (region.parent,)
                    )
        ...
        try:
            ...
            for jurisdiction in jurisdictions.values():
                ...
                cur.execute(
                    ...
                    (jurisdiction.region_slug,),
                )
                region_row = cur.fetchone()
                if not region_row:
                    logger.error(
                        f"Region not found for jurisdiction {jurisdiction.slug}: {jurisdiction.region_slug}"
                    )
                    continue
                ...
                logger.info(
                    f"Synced jurisdiction: {jurisdiction.slug} -> {jurisdiction_id}"
                )
        ...
        try:
            ...
            for plan in plans.values():
                ...
                for region_slug in plan.regions:
                    cur.execute(
                        "SELECT id FROM regions WHERE slug = %s", (region_slug,)
                    )
                    region_row = cur.fetchone()
                    if region_row:
                        cur.execute(
    ...
    def test_jurisdiction_config(self, jurisdiction_slug: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "permits": [
                    permit.dict() for permit in permits[:3]
                ],  # First 3 for preview
            }
...
def main():
    ...
    sync_parser.add_argument(
        "--jurisdictions", action="store_true", help="Sync jurisdictions"
    )
    ...
    list_parser.add_argument(
        "type", choices=["regions", "jurisdictions"], help="Entity type to list"
    )
    ...
    status_parser.add_argument(
        "--inactive", action="store_true", help="Set as inactive"
    )
