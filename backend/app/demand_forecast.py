...
class SurgeLabeler:
    ...
    def generate_surge_labels(
        self, region_id: str, start_date: date, end_date: date
    ) -> pd.DataFrame:
        """
        ...
        """
        logger.info(
            f"Generating surge labels for region {region_id}: {start_date} to {end_date}"
        )
        ...
        weekly_data["percentile_rank"] = (
            weekly_data["total_activity"].rank(pct=True) * 100
        )

        # Label surge weeks
        weekly_data["is_surge"] = (
            weekly_data["percentile_rank"] >= self.config.surge_percentile_threshold
        )
        weekly_data["surge_threshold_percentile"] = (
            self.config.surge_percentile_threshold
        )
        ...
        logger.info(
            f"Generated {len(weekly_data)} weekly labels, {weekly_data['is_surge'].sum()} surge weeks"
        )
        return weekly_data
    ...
    def _get_weekly_activity(
        ...
        try:
            ...
            permits_result = self.supabase.rpc(
                "sql_query", {"query": permits_query}
            ).execute()
            permits_df = pd.DataFrame(
                permits_result.data if permits_result.data else []
            )

            if permits_df.empty:
                # Create empty weekly structure if no data
                date_range = pd.date_range(start=start_date, end=end_date, freq="W-MON")
                permits_df = pd.DataFrame({"week_start": date_range, "permit_count": 0})
            else:
            ...
            permits_df["total_activity"] = (
                permits_df["permit_count"]
                + permits_df["violation_count"]
                + permits_df["bid_count"]
            )
        ...
        try:
            ...
            for _, row in weekly_data.iterrows():
                record = {
                    ...
                    "surge_threshold_percentile": float(
                        row["surge_threshold_percentile"]
                    ),
                }
...
class FeatureEngineer:
    ...
    def generate_features(
        ...
        """
        logger.info(
            f"Generating features for region {region_id}: {start_date} to {end_date}"
        )
        ...
        logger.info(
            f"Generated {len(features_df)} feature records with {len(features_df.columns)} features"
        )
        return features_df

    def _get_permit_time_series(
        self, region_id: str, start_date: date, end_date: date
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                ...
                df = pd.DataFrame({"date": date_range, "permit_count": 0})
            else:
                ...
                df = (
                    df.set_index("date").reindex(full_range, fill_value=0).reset_index()
                )
    ...
    def _create_lagged_features(self, permit_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in self.config.rolling_windows:
            weekly_df[f"permits_ma_{window}w"] = (
                weekly_df["permit_count"].rolling(window=window, min_periods=1).mean()
            )

        # Create trend features (slope of recent periods)
        for window in [4, 12]:
            weekly_df[f"permits_trend_{window}w"] = (
        ...
        weekly_df["permits_seasonal_index"] = (
            weekly_df["permit_count"]
            / weekly_df["permit_count"]
            .rolling(window=52, min_periods=26, center=True)
            .mean()
        ).fillna(1.0)
        ...
        try:
            ...
            for _, row in features_df.iterrows():
                ...
                feature_columns = [
                    ...
                    if col not in ["date", "permit_count"]
                ]
                for col in feature_columns:
                    if pd.notna(row[col]):
                        record[col] = (
                            float(row[col])
                            if isinstance(row[col], (int, float, np.number))
                            else row[col]
                        )
            ...
            if records:
                result = (
                    self.supabase.table("forecast_features").upsert(records).execute()
                )
                logger.info(
                    f"Stored {len(records)} feature records for region {region_id}"
                )
...
class DemandSurgeForecaster:
    ...
    def train_models(self, region_id: str, end_date: date = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"Training models for region {region_id}: {start_date} to {end_date}"
        )

        # Generate surge labels and features
        surge_labels = self.surge_labeler.generate_surge_labels(
    ...
    def _prepare_training_data(
        self, features: pd.DataFrame, surge_labels: pd.DataFrame
        ...
        merged = weekly_features.merge(
            surge_labels[["week_start", "is_surge"]], on="week_start", how="inner"
        )
    ...
    def _train_gradient_boost(
        self, X: pd.DataFrame, y: pd.Series, region_id: str
    ) -> Dict[str, Any]:
        ...
        # Store models
        model_version = (
            f"gradient_boost_{region_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _train_lightgbm(
        ...
        # Train final model
        lgb_model = lgb.train(
            params, train_data, num_boost_round=len(cv_results["valid auc-mean"])
        )

        # Store model
        model_version = (
            f"lightgbm_{region_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _train_xgboost(
        ...
        # Train final model
        xgb_model = xgb.train(params, dtrain, num_boost_round=len(cv_results))

        # Store model
        model_version = (
            f"xgboost_{region_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _store_model_performance(self, results: Dict[str, Any], region_id: str):
        ...
        try:
            ...
            if records:
                result = (
                    self.supabase.table("model_performance").upsert(records).execute()
                )
    ...
    def generate_forecast(
        self, region_id: str, target_date: date = None, model_version: str = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Generating forecast for region {region_id}, week {target_week_start}"
        )
    ...
    def _predict_with_model(
        ...
        return {
            ...
            "calibration_method": (
                "isotonic" if model_type == "gradient_boost" else "none"
            ),
        }
    ...
    def _store_prediction(self, prediction_record: Dict[str, Any]):
        ...
        try:
            result = (
                self.supabase.table("forecast_predictions")
                .upsert([prediction_record])
                .execute()
            )
            logger.info(
                f"Stored prediction for region {prediction_record['region_id']}"
            )
