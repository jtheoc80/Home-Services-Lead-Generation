...
class LeadsAPI:
            ...
            if (lat is not None and lon is not None and radius_km is not None 
                ...
                for lead in leads:
                    if lead['lat'] and lead['lon']:
                        distance = self._haversine_distance(lat, lon, lead['lat'], lead['lon'])
    ...
    def get_jurisdictions(self, region_slug: Optional[str] = None) -> List[Dict[str, Any]]:
...
def main():
    ...
    parser.add_argument("--list-jurisdictions", action="store_true", help="List all jurisdictions")
