...
class LeadScorer:
    ...
    def calculate_decay_factor(self, event_date: datetime, half_life_days: int = 90) -> float:
        """
    ...
    def get_personalized_weights(self, account_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error(f"Error calculating personalized weights for user {account_id}: {e}")
            return {}
    ...
    def calculate_base_score(self, lead: Dict) -> float:
        ...
        trade_scores = {
            'roofing': 25, 'kitchen': 24, 'bath': 22, 'pool': 20, 'fence': 15,
            'windows': 18, 'foundation': 22, 'solar': 20, 'hvac': 18,
            'electrical': 16, 'plumbing': 16
        }
        
        max_trade_score = 0
        if lead.get('trade_tags'):
            max_trade_score = max([trade_scores.get(tag, 0) for tag in lead['trade_tags']] + [0])
