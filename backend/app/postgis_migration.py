...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check PostGIS availability"
    )
    parser.add_argument("--enable", action="store_true", help="Enable PostGIS support")
    parser.add_argument(
        "--migrate", action="store_true", help="Run full PostGIS migration"
    )
    parser.add_argument(
        "--force", action="store_true", help="Force operation even if USE_POSTGIS=false"
    )
