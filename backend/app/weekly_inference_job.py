...
from app.demand_forecast import get_demand_surge_forecaster, ForecastConfig
...
class WeeklyInferenceJob:
    ...
    async def run_weekly_inference(self, target_date: Optional[date] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            forecast_results = await self._generate_region_forecasts(regions, target_date)
            ...
            results = {
                ...
                "impact_report_summary": impact_report.get('summary') if impact_report else None
            }
            
            logger.info(f"Weekly inference job completed successfully in {job_duration:.1f}s")
            return results
    ...
    async def _get_active_regions(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.supabase.table('regions').select('*').eq('active', True).execute()
    ...
    async def _generate_region_forecasts(self, regions: List[Dict[str, Any]], target_date: date) -> Dict[str, List]:
        ...
        for region in regions:
            try:
                ...
                # Generate forecast
                forecast_result = self.forecaster.generate_forecast(region_id, target_date)
                ...
                logger.info(f"Generated forecast for {region_slug}: p_surge={forecast_result['p_surge']:.3f}")
    ...
    async def _update_gold_forecasts(self, forecast_results: Dict[str, List], target_date: date):
        ...
        try:
            ...
            for result in forecast_results['successful']:
                ...
                try:
                    prior_result = self.supabase.rpc('sql_query', {'query': prior_query}).execute()
                    prior_year_p_surge = prior_result.data[0]['p_surge'] if prior_result.data else None
                except:
                ...
                if prior_year_p_surge:
                    surge_risk_change_pct = ((forecast['p_surge'] - prior_year_p_surge) / prior_year_p_surge) * 100
            ...
            if gold_records:
                result = self.supabase.table('gold.forecast_nowx').upsert(gold_records).execute()
                logger.info(f"Updated {len(gold_records)} records in gold.forecast_nowx")
    ...
    async def _generate_impact_report(self, target_date: date) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if report and 'error' not in report:
                ...
                logger.info(f"Generated impact report with {report.get('total_regions', 0)} regions")
                return report
            else:
    ...
    async def _complete_job_tracking(self, job_id: str, forecast_results: Dict[str, List], duration_seconds: float):
        """Complete job tracking with results"""
        try:
            regions_processed = [r['region_slug'] for r in forecast_results['successful']]
            models_used = list(set([r['forecast'].get('model_version', 'unknown') for r in forecast_results['successful']]))
            ...
            self.supabase.table('forecast_jobs').update(update_data).eq('id', job_id).execute()
            logger.info(f"Completed job tracking for job {job_id}")
            
        except Exception as e:
            logger.error(f"Error completing job tracking: {str(e)}")
    
    async def _fail_job_tracking(self, job_id: str, error_message: str, duration_seconds: float):
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Run weekly demand surge inference job')
