...
async def get_demand_index(
    ...
    try:
        ...
        if not result.data:
            ...
            logger.warning("No forecast data found in gold.forecast_nowx, generating fresh forecasts")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving demand index: {str(e)}"
        )
...
async def get_region_demand_index(
    ...
    try:
        ...
        if not result.data:
            ...
            logger.info(f"No forecast found for region {region_slug}, attempting to generate")
            
            # Get region ID
            region_result = supabase.table('regions').select('*').eq('slug', region_slug).execute()
            
            if not region_result.data:
                raise HTTPException(
                    status_code=404,
                    detail=f"Region {region_slug} not found"
                )
            ...
            # Store in gold table
            await _update_gold_forecast(region_slug, region_result.data[0]['name'], forecast_result)
        ...
        logger.info(f"Returned demand index for region {region_slug}: score={response.score:.3f}")
        return response
...
async def refresh_demand_index(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error refreshing demand index: {str(e)}"
        )
...
async def _generate_fresh_forecasts():
    ...
    try:
        ...
        regions_result = supabase.table('regions').select('*').eq('active', True).execute()
        ...
        for region_data in regions_result.data:
            try:
                ...
                logger.error(f"Error generating forecast for region {region_slug}: {str(e)}")
                
    except Exception as e:
        logger.error(f"Error generating fresh forecasts: {str(e)}")

async def _update_gold_forecast(region_slug: str, region_name: str, forecast_result: Dict[str, Any]):
    ...
    try:
        ...
        if prior_year_p_surge:
            surge_risk_change_pct = ((forecast_result['p_surge'] - prior_year_p_surge) / prior_year_p_surge) * 100
