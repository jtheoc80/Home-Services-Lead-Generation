...
class LeadMLTrainer:
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["feedback_age_days"] = (
            datetime.now() - pd.to_datetime(df["feedback_date"])
        ).dt.days
        ...
        df["source_avg_cancellation_score"] = df[
    ...
    def train_model(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "feature_importance": (
                dict(zip(self.feature_columns, base_model.feature_importances_))
        ...
        logger.info(
            f"Model trained - Accuracy: {metrics['accuracy']:.3f}, AUC: {metrics['roc_auc']:.3f}"
        )
        return metrics
    ...
    def run_training(self, model_dir: str = "./models") -> Dict[str, Any]:
        ...
        try:
            ...
            if len(df) < 50:  # Minimum samples needed
                logger.warning(
                    f"Insufficient training data: {len(df)} samples. Skipping training."
                )
                return {
                    "status": "skipped",
            ...
            return {"status": "success", "model_file": model_file, **metrics}
