        ...
        try:
            ...
            self.jinja_env = Environment(
                loader=FileSystemLoader(
                    [str(EMAIL_TEMPLATE_DIR), str(SMS_TEMPLATE_DIR)]
                ),
            ...
            try:
                if isinstance(context["effective_at"], str):
                    effective_date = datetime.fromisoformat(
                        context["effective_at"].replace("Z", "+00:00")
                    )
                else:
        ...
        if "reactivated_at" in context:
            try:
                if isinstance(context["reactivated_at"], str):
                    reactivated_date = datetime.fromisoformat(
                        context["reactivated_at"].replace("Z", "+00:00")
                    )
                else:
                ...
                common_context.update(
                    {
                        ...
                        "next_billing_date": (
                            reactivated_date.replace(month=reactivated_date.month + 1)
                            if reactivated_date.month < 12
                            else reactivated_date.replace(
                                year=reactivated_date.year + 1, month=1
                            )
                        ).strftime("%B %d, %Y"),
                    }
                )
