...
class ExportController:
    ...
    def process_export_request(self, request: ExportRequest) -> ExportResult:
        ...
        allowed, reason = self.is_export_allowed(
            request.export_type, request.requester, request.parameters
        )
    ...
    def _log_blocked_export_attempt(
        self,
        export_type: ExportType,
        requester: str,
        reason: str,
    ...
    def _notify_blocked_export_attempt(
        self, export_type: ExportType, requester: str, reason: str
    ):
        ...
        try:
            ...
            if admin_emails:
                logger.info(
                    f"Would notify admins about blocked export: {requester} attempted {export_type.value}"
                )
