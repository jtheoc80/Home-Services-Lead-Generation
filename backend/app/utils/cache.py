...
class RedisCache:
    ...
    def _get_client(self):
        ...
        if self._client is None and self.config.redis_url:
            try:
                ...
                logger.warning(
                    f"Failed to connect to Redis: {str(e)}. Caching disabled."
                )
                return None
        return self._client
    ...
    def cache_lead_scores(
        self, lead_id: int, scores: Dict[str, Any], ttl: int = 1800
    ) -> bool:
        """
    ...
    def cache_export_result(
        self, export_id: str, result: Dict[str, Any], ttl: int = 7200
    ) -> bool:
        """
