...
class SubscriptionNotificationService:
    ...
    def _should_send_sms(self, event_data: SubscriptionEventData) -> bool:
        ...
        return (
            event_data.sms_enabled and 
            event_data.phone_verified and 
            event_data.customer_phone and
            self.notification_service.sms.is_enabled()
        )
    
    def _get_template_context(self, event_data: SubscriptionEventData) -> Dict[str, Any]:
    ...
    def send_cancellation_confirmation(self, event_data: SubscriptionEventData) -> Dict[str, bool]:
        """
        ...
        if self.notification_service.email.is_enabled():
            try:
                ...
                if success:
                    logger.info(f"Cancellation confirmation email sent to {event_data.customer_email}")
                else:
                    logger.error(f"Failed to send cancellation confirmation email to {event_data.customer_email}")
        ...
        if self._should_send_sms(event_data):
            try:
                ...
                success = self.notification_service.sms.send_sms(
                    to_number=event_data.customer_phone,
                    message=sms_content
                )
                results["sms"] = success
                
                if success:
                    logger.info(f"Cancellation confirmation SMS sent to {event_data.customer_phone}")
                else:
                    logger.error(f"Failed to send cancellation confirmation SMS to {event_data.customer_phone}")
    ...
    def send_grace_period_reminder(self, event_data: SubscriptionEventData) -> Dict[str, bool]:
        """
        ...
        if self.notification_service.email.is_enabled():
            try:
                ...
                if success:
                    logger.info(f"Grace period reminder email sent to {event_data.customer_email}")
                else:
                    logger.error(f"Failed to send grace period reminder email to {event_data.customer_email}")
        ...
        if self._should_send_sms(event_data):
            try:
                ...
                if success:
                    logger.info(f"Grace period reminder SMS sent to {event_data.customer_phone}")
                else:
                    logger.error(f"Failed to send grace period reminder SMS to {event_data.customer_phone}")
    ...
    def send_access_ended(self, event_data: SubscriptionEventData) -> Dict[str, bool]:
        ...
        if self.notification_service.email.is_enabled():
            try:
                ...
                if success:
                    logger.info(f"Access ended email sent to {event_data.customer_email}")
                else:
                    logger.error(f"Failed to send access ended email to {event_data.customer_email}")
        ...
        if self._should_send_sms(event_data):
            try:
                ...
                if success:
                    ...
                    logger.error(f"Failed to send access ended SMS to {event_data.customer_phone}")
    ...
    def send_reactivation_confirmation(self, event_data: SubscriptionEventData) -> Dict[str, bool]:
        """
        ...
        if self.notification_service.email.is_enabled():
            try:
                ...
                if success:
                    logger.info(f"Reactivation confirmation email sent to {event_data.customer_email}")
                else:
                    logger.error(f"Failed to send reactivation confirmation email to {event_data.customer_email}")
        ...
        if self._should_send_sms(event_data):
            try:
                ...
                if success:
                    logger.info(f"Reactivation confirmation SMS sent to {event_data.customer_phone}")
                else:
                    logger.error(f"Failed to send reactivation confirmation SMS to {event_data.customer_phone}")
