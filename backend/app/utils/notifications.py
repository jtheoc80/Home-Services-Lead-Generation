    ...
    def _get_client(self):
        ...
        if self._client is None and self.config.sendgrid_api_key:
            try:
                ...
                logger.warning(
                    "SendGrid library not installed. Email notifications disabled."
                )
                return None
        return self._client
...
class SMSNotifier:
    ...
    def _get_client(self):
        ...
        if self._client is None and self.config.twilio_sid and self.config.twilio_token:
            try:
                ...
                logger.warning(
                    "Twilio library not installed. SMS notifications disabled."
                )
                return None
        return self._client
    ...
    def is_enabled(self) -> bool:
        ...
        return bool(
            ...
            and self.config.twilio_from
            and self._get_client()
        )
    ...
    def send_sms(self, to_number: str, message: str) -> bool:
        ...
        try:
            ...
            message_obj = client.messages.create(
                body=message, from_=self.config.twilio_from, to=to_number
            )
...
class NotificationService:
    ...
    def notify_lead_processed(
        ...
        if email_recipients and self.email.is_enabled():
            for email in email_recipients:
                success = self.email.send_email(
                    to_email=email, subject="Lead Processing Complete", content=message
                )
        ...
        if sms_recipients and self.sms.is_enabled():
            for phone in sms_recipients:
                success = self.sms.send_sms(to_number=phone, message=message)
    ...
    def notify_export_request(
        ...
        message = (
            f"Export request by {requester} for {export_type} was {status.lower()}."
        )

        if email_recipients and self.email.is_enabled():
            for email in email_recipients:
                return self.email.send_email(
                    to_email=email, subject=subject, content=message
                )
