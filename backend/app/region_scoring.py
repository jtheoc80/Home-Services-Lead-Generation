...
class RegionAwareScoring:
        ...
        for state in df["state"].dropna().unique():
            ...
            if len(state_data) >= 10:  # Minimum sample size
                try:
                    ...
                    logger.info(
                        f"Trained calibrator for state {state} with {len(state_data)} samples"
                    )
                except Exception as e:
        ...
        for region_slug in df["region_slug"].dropna().unique():
            ...
            if len(region_data) >= 20:  # Higher threshold for region-specific
                try:
                    ...
                    logger.info(
                        f"Trained calibrator for region {region_slug} with {len(region_data)} samples"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to train calibrator for region {region_slug}: {e}"
                    )
        ...
        for region_slug in df["region_slug"].dropna().unique():
            ...
            if len(region_data) >= 10:
                ...
                if baseline_conversion > 0:
                    ...
                    logger.info(
                        f"Region {region_slug}: conversion {region_conversion:.3f} vs baseline {baseline_conversion:.3f}, adjustment {adjustment:.3f}"
                    )
    ...
    def apply_region_calibration(
        ...
        if region_slug and region_slug in self.region_calibrators:
            ...
            logger.debug(
                f"Applied region calibration for {region_slug}: {lead_score} -> {calibrated_score}"
            )
        ...
        if state and state in self.state_calibrators:
            ...
            calibrated_score = calibrator.predict([lead_score])[0] * 100
            logger.debug(
                f"Applied state calibration for {state}: {lead_score} -> {calibrated_score}"
            )
            return calibrated_score
    ...
    def apply_regional_adjustment(
        self, lead_score: float, region_slug: Optional[str] = None
    ) -> float:
        ...
        if region_slug and region_slug in self.regional_score_adjustments:
            ...
            logger.debug(
                f"Applied regional adjustment for {region_slug}: {lead_score} -> {adjusted_score}"
            )
    ...
    def load_calibration_model(self, model_path: str) -> bool:
        ...
        try:
            ...
            self.regional_score_adjustments = model_data.get(
                "regional_score_adjustments", {}
            )

            logger.info(f"Loaded calibration model from {model_path}")
            logger.info(
                f"Loaded {len(self.region_calibrators)} region calibrators, {len(self.state_calibrators)} state calibrators"
            )
            return True
...
def main():
    ...
    parser.add_argument(
        "--train", action="store_true", help="Train regional calibrators"
    )
    ...
    parser.add_argument(
        "--stats", action="store_true", help="Show calibration model stats"
    )
    ...
    try:
        ...
        if args.score is not None:
            calibrated = scorer.apply_region_calibration(
                args.score, args.state, args.region
            )
