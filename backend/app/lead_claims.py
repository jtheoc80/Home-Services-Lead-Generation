...
async def claim_lead(
    ...
    try:
        ...
        lead_result = (
            client.table("leads")
            .select("id, jurisdiction, permit_id, address")
        ...
        logger.info(
            f"User {user.account_id} claimed lead {request.lead_id}, {remaining_credits} credits remaining"
        )
        ...
        logger.error(
            f"Failed to claim lead {request.lead_id} for user {user.account_id}: {e}"
        )
