...
SEND_CANCEL_REMINDER_DAYS = int(
    os.getenv("SEND_CANCEL_REMINDER_DAYS", "1")
...
class CancellationNotificationJob:
    ...
    def get_subscriptions_for_grace_reminder(self) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Found {len(mock_subscriptions)} subscriptions needing grace reminders"
        )
        ...
        for sub_data in subscriptions:
            try:
                ...
                event_data = SubscriptionEventData(
                    ...
                    canceled_at=(
                        datetime.fromisoformat(sub_data["canceled_at"])
                ...
                if DRY_RUN:
                    logger.info(
                        f"DRY RUN: Would send grace reminder to {event_data.customer_email}"
                    )
                    continue

                # Send notifications
                results = self.notification_service.send_grace_period_reminder(
                    event_data
                )
                ...
                if results.get("email") or results.get("sms"):
                    ...
                    logger.info(
                        f"Grace reminder sent successfully to {event_data.customer_email}"
                    )
                    ...
                    self.error_count += 1
                    logger.error(
                        f"Failed to send grace reminder to {event_data.customer_email}"
                    )

            except Exception as e:
                self.error_count += 1
                logger.error(
                    f"Error processing grace reminder for {sub_data.get('customer_email', 'unknown')}: {e}"
                )
    ...
    def process_access_ended(self) -> None:
        ...
        for sub_data in subscriptions:
            try:
                ...
                if DRY_RUN:
                    logger.info(
                        f"DRY RUN: Would send access ended notification to {event_data.customer_email}"
                    )
                    continue
                ...
                if results.get("email") or results.get("sms"):
                    self.processed_count += 1
                    logger.info(
                        f"Access ended notification sent successfully to {event_data.customer_email}"
                    )
                    ...
                    self.error_count += 1
                    logger.error(
                        f"Failed to send access ended notification to {event_data.customer_email}"
                    )

            except Exception as e:
                self.error_count += 1
                logger.error(
                    f"Error processing access ended notification for {sub_data.get('customer_email', 'unknown')}: {e}"
                )
    ...
    def run(self) -> Dict[str, int]:
        ...
        logger.info(
            f"Cancellation notification job completed in {duration.total_seconds():.2f}s"
        )
