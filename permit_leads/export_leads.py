...
def compute_enhanced_score(
    record: Dict[str, Any], now: datetime, lookback_days: int
) -> Dict[str, Any]:
    """
...
def export_enriched_leads(
    db_path: Path, out_dir: Path, lookback_days: int, enrich_data: bool = True
) -> Tuple[Path, int]:
    """
    ...
    for row in rows:
        ...
        if enrich_data and not record.get("latitude"):
            try:
                ...
                print(
                    f"Enrichment failed for {record.get('permit_id', 'unknown')}: {e}"
                )
