...
class HoustonCityScraper(BaseScraper):
    ...
    def fetch_permits(
        ...
        logger.warning(
            "Live Houston City permit scraping not yet implemented - using sample data"
        )
        return self._fetch_sample_data(limit)
    ...
    def _fetch_live_data(
        ...
        logger.info(
            f"TODO: Implement live scraping from {search_url} with params {params}"
        )

        # For now, fall back to sample data
        return self._fetch_sample_data(limit)

    def _parse_permit_table(
        self, html_content: str, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def parse_permit(self, raw_data: Dict[str, Any]) -> Optional[PermitRecord]:
        ...
        try:
            ...
            value = (
                safe_float(valuation_str.replace("$", "").replace(",", ""))
                if valuation_str
                else None
            )
