...
def test_adapter_initialization(mock_jurisdiction):
    ...
    assert adapter.base_url == 'https://test.arcgis.com/rest/services/TestService/FeatureServer/0'
...
def test_rate_limiting(adapter):
    ...
    assert elapsed >= expected_min_time, f"Rate limiting too fast: {elapsed} < {expected_min_time}"
...
def test_jitter_on_429(mock_sleep, adapter):
    ...
    with patch.object(adapter.session, 'get', return_value=mock_response) as mock_get:
        ...
        retry_sleeps = [s for s in sleep_calls if s > 0.5]  # Filter out rate limiting sleeps
...
def test_get_total_count(adapter):
    ...
    with patch.object(adapter, '_make_request_with_jitter', return_value=mock_count_response) as mock_request:
...
def test_get_total_count_logging(adapter, caplog):
    ...
    with caplog.at_level('INFO'):
        with patch.object(adapter, '_make_request_with_jitter', return_value=mock_count_response):
            adapter._get_total_count("test_query")
            
            # Check that the count was logged
            assert "Total ArcGIS record count for query 'test_query': 5678" in caplog.text
...
def test_scrape_permits_respects_max_record_count(mock_logger, adapter):
    ...
    with patch.object(adapter, '_make_request_with_jitter', side_effect=mock_request_side_effect):
        since = datetime.now() - timedelta(days=1)
        permits = adapter.scrape_permits(since, limit=None)
        
        # Check that maxRecordCount was logged
        info_calls = [call for call in mock_logger.info.call_args_list if 'maxRecordCount' in str(call)]
...
def test_request_throttling_actual_timing():
    ...
    mock_jurisdiction = Jurisdiction(
        slug='test', name='Test', region_slug='test', state='TX',
        fips=None, timezone='America/Chicago', provider='arcgis', active=True,
        source_config={'url': 'https://test.com/service/0', 'date_field': 'date', 'field_map': {}}
    )
    adapter = ArcGISAdapter(mock_jurisdiction)
...
def test_error_handling_in_scrape_permits(adapter):
    ...
    with patch.object(adapter, '_make_request_with_jitter', side_effect=Exception("Test error")):
        since = datetime.now() - timedelta(days=1)
        permits = adapter.scrape_permits(since, limit=None)
