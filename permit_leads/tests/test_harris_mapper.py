...
class TestConvertFeatureToDict:
    ...
    def test_empty_feature(self):
        ...
        assert all(result[key] == 0 if key == 'event_id' else result[key] == "" 
                  for key in result if key != 'raw')
        assert result['raw'] == feature
    ...
    def test_invalid_feature_type(self):
        ...
        assert result['event_id'] == 0
        assert all(result[key] == "" for key in result if key not in ['event_id', 'raw'])
        assert result['raw'] == {}
...
class TestParseAddress:
    ...
    def test_complex_numbers(self):
        ...
        test_cases = [
            ...
            ("123 1/2 Pine Ave", "123", "1/2 Pine Ave"),  # Note: 1/2 becomes part of street name
        ]
...
class TestIntegration:
    ...
    def test_realistic_harris_county_feature(self):
        ...
        required_keys = {
            'event_id', 'permit_number', 'permit_name', 'app_type', 
            'issue_date', 'project_number', 'full_address', 
            'street_number', 'street_name', 'status', 'raw'
        }
