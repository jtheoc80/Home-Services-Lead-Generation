...
def test_registry_yaml_valid():
    ...
    assert region_slugs == expected_regions, f"Expected regions {expected_regions}, got {region_slugs}"
    ...
    assert active_slugs == expected_active, f"Expected active jurisdictions {expected_active}, got {active_slugs}"
    
    # Verify all active jurisdictions have required fields
    for jurisdiction in active_jurisdictions:
        assert "name" in jurisdiction, f"Jurisdiction {jurisdiction['slug']} missing name"
        assert "state" in jurisdiction, f"Jurisdiction {jurisdiction['slug']} missing state"
        assert "region" in jurisdiction, f"Jurisdiction {jurisdiction['slug']} missing region"
        assert "provider" in jurisdiction, f"Jurisdiction {jurisdiction['slug']} missing provider"
        assert "source_config" in jurisdiction, f"Jurisdiction {jurisdiction['slug']} missing source_config"
        
        # Verify source_config structure
        source_config = jurisdiction["source_config"]
        assert "url" in source_config, f"Jurisdiction {jurisdiction['slug']} missing source URL"
        assert "date_field" in source_config, f"Jurisdiction {jurisdiction['slug']} missing date_field"
        assert "field_map" in source_config, f"Jurisdiction {jurisdiction['slug']} missing field_map"
        
        # All active jurisdictions should be in Houston Metro
        assert jurisdiction["region"] == "tx-houston", f"Active jurisdiction {jurisdiction['slug']} should be in tx-houston region"
        assert jurisdiction["state"] == "TX", f"Active jurisdiction {jurisdiction['slug']} should be in TX"
        assert jurisdiction["provider"] == "arcgis", f"Active jurisdiction {jurisdiction['slug']} should use arcgis provider"
...
def test_registry_yaml_field_mappings():
    ...
    for jurisdiction in active_jurisdictions:
        ...
        assert mapped_fields == expected_fields, f"Jurisdiction {jurisdiction['slug']} has inconsistent field mapping: {mapped_fields} vs {expected_fields}"