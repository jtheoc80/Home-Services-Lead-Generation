...
class SupabaseSink:
    ...
    def __init__(
        self, upsert_table: str, conflict_col: str = "event_id", chunk_size: int = 500
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"Initialized SupabaseSink for table '{upsert_table}' with conflict column '{conflict_col}'"
            )
    ...
    def upsert_batch(self, records: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        try:
            ...
            response = (
                self.client.table(self.upsert_table)
                .upsert(serialized_records, on_conflict=self.conflict_col)
                .execute()
            )
            ...
            logger.info(
                f"Successfully upserted {success_count} records to {self.upsert_table}"
            )
    ...
    def upsert_batch_rpc(self, records: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        try:
            response = self.client.rpc(
                "upsert_permit", {"p": serialized_records}
            ).execute()

            # Check response status
            if hasattr(response, "status_code") and response.status_code != 200:
                logger.error(
                    f"Batch RPC upsert failed with status {response.status_code}"
                )
                return {"success": 0, "failed": len(records)}

            success_count = len(records)
            logger.info(
                f"Successfully upserted {success_count} records via batch RPC to {self.upsert_table}"
            )
            return {"success": success_count, "failed": 0}

        except Exception as e:
            logger.error(
                f"Failed to upsert batch of {len(records)} records via RPC: {e}"
            )
            return {"success": 0, "failed": len(records)}

    def upsert_records(
        self, records: List[Dict[str, Any]], use_rpc: bool = True
    ) -> Dict[str, int]:
        """
        ...
        if use_rpc:
            ...
            logger.info(
                f"Starting table upsert of {len(records)} records in {total_chunks} chunks of {self.chunk_size}"
            )
            ...
            for i in range(0, len(records), self.chunk_size):
                ...
                logger.info(
                    f"Processing chunk {chunk_num}/{total_chunks} ({len(chunk)} records)"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Chunk {chunk_num} complete: {result['success']} success, {result['failed']} failed"
                    )
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            response = (
                self.client.table(self.upsert_table).select("*").limit(1).execute()
            )
