...
class RegionAwareAdapter:
    ...
    def annotate_with_region_info(
        self, permit: PermitRecord, jurisdiction: Jurisdiction, region: Region
    ) -> PermitRecord:
    ...
    def scrape_jurisdiction(
        ...
        if not region:
            raise ValueError(
                f"Region not found for jurisdiction: {jurisdiction.region_slug}"
            )

        logger.info(
            f"Scraping {jurisdiction.name} ({jurisdiction.provider}) since {since}"
        )
        ...
        try:
            ...
            for permit in permits:
                annotated_permit = self.annotate_with_region_info(
                    permit, jurisdiction, region
                )
                annotated_permits.append(annotated_permit)

            logger.info(
                f"Scraped {len(annotated_permits)} permits from {jurisdiction.name}"
            )
    ...
    def scrape_all_jurisdictions(
        self, since: datetime, limit: Optional[int] = None, max_retries: int = 3
    ) -> Dict[str, List[PermitRecord]]:
        """Scrape permits from all active jurisdictions."""
        jurisdictions = self.get_active_jurisdictions()
        results = {}

        for jurisdiction in jurisdictions:
            permits = self.scrape_jurisdiction(
                jurisdiction.slug, since, limit, max_retries=max_retries
            )
