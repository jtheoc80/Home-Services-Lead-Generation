...
def run_source(source_cfg, session, days, limit, storage):
    ...
    if not adapter_cls:
        print(f"[WARN] Unsupported adapter type: {adapter_type} for {name}", file=sys.stderr)
        return
    ...
    for rec in raw_records:
        ...
        text_blob = " ".join([str(rec_norm.get(k, "")) for k in ["description", "work_class", "permit_type"]]).lower()
    ...
    print(f"[{name}] pulled={count_in} residential_like={count_res} saved={count_saved}")

def main():
    ap = argparse.ArgumentParser(description="Scrape/collect building permits for residential projects and export as contractor leads.")
    ap.add_argument("--config", default="config/sources.yaml", help="Path to YAML config of sources.")
    ...
    ap.add_argument("--db", default="permits.sqlite", help="SQLite db filename inside outdir.")
    ap.add_argument("--csv", default="permits.csv", help="CSV filename inside outdir.")
    ap.add_argument("--user_agent", default="PermitLeadBot/1.0 (+contact@example.com)", help="User-Agent for polite scraping.")
    args = ap.parse_args()
