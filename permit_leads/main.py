...
def write_json_summary(
    summary_path: str, record_count: int, sources_processed: List[str], success: bool
) -> None:
    """
    ...
    for source in source_list:
        if source in source_to_jurisdiction:
            ...
            if jurisdiction is None:
                logger.warning(
                    f"Source '{source}' maps to inactive jurisdiction, skipping"
                )
    ...
    try:
        ...
        for permit in permits:
            ...
            payload = {
                ...
                or permit_dict.get("jurisdiction", "unknown"),
                "created_at": "now()",
...
""",
    ...
    parser.add_argument(
        "--sources",
        help="Comma-separated list of sources (e.g., 'austin,dallas,harris_county')",
    )
    parser.add_argument(
        "--sink",
        choices=["supabase", "csv", "sqlite", "jsonl"],
        help="Primary output destination",
    )
    ...
    try:
        ...
        if args.command == "__auto__":
            if (
                ...
                and not use_multi_source
            ):
                raise SystemExit(
                    "Must specify either --source, --all, --region-aware, --jurisdiction, or --sources (scrape mode)."
                )
            ...
            if (
                ...
                raise SystemExit(
                    "Must specify either --source, --all, --region-aware, --jurisdiction, or --sources"
                )
        ...
        if use_multi_source:
            ...
            for jurisdiction in jurisdictions:
                ...
                permits, jur_results = run_region_aware_scraper(
