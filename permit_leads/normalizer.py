...
class PermitNormalizer:
    ...
    WORK_TYPE_PATTERNS = {
        "residential": [
            ...
            r"renovation",
            r"kitchen",
            r"bathroom",  # Added 'renovation' for residential
            r"garage",
            r"deck",
            r"porch",
            r"fence",
    ...
    def normalize_record(
        ...
        try:
            ...
            jurisdiction = self._normalize_jurisdiction(
                source_config.get("jurisdiction")
            )
            ...
            normalized["permit_id"] = self._extract_permit_id(
                raw_record, mappings, jurisdiction
            )
            ...
            normalized["work_type"] = self._normalize_work_type(
                ...
                or normalized.get("permit_category")
                or normalized.get("work_class"),
            )
            ...
            normalized["work_description"] = self._clean_text(
                normalized.get("work_description")
            )
            normalized["applicant_name"] = self._clean_name(
                normalized.get("applicant_name")
            )
            normalized["contractor_name"] = self._clean_name(
            ...
            self.stats["by_jurisdiction"][jurisdiction] = (
                self.stats["by_jurisdiction"].get(jurisdiction, 0) + 1
            )
    ...
    def normalize_batch(
        self, raw_records: List[Dict[str, Any]], source_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Normalized {len(normalized_records)} out of {len(raw_records)} records"
        )
    ...
    def _apply_field_mappings(
        self, raw_record: Dict[str, Any], mappings: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def _get_mapped_value(
        self, raw_record: Dict[str, Any], mappings: Dict[str, str], field: str
    ) -> Any:
    ...
    def _extract_permit_id(
        self, raw_record: Dict[str, Any], mappings: Dict[str, str], jurisdiction: str
    ) -> str:
        """Extract and normalize permit ID."""
        permit_id = self._get_mapped_value(
            raw_record, mappings, "permit_number"
        ) or self._get_mapped_value(raw_record, mappings, "permit_id")
    ...
    def _normalize_work_type(self, description: str, category: str) -> str:
        ...
        text = " ".join(
            filter(None, [str(description or ""), str(category or "")])
        ).lower()
