...
"""
    ...
    def fetch_since(
        ...
        if not csv_files:
            logger.warning(
                f"No CSV files found for {self.jurisdiction}. "
                    ...
                    for row in reader:
                        ...
                        if since and date_field in mapped_row:
                            try:
                                ...
                                logger.warning(
                                    f"Could not parse date {mapped_row[date_field]}: {e}"
                                )
                                continue
    ...
    def _apply_mappings(
        self, row: Dict[str, Any], mappings: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    def fetch(self, since_days: int) -> Iterable[bytes | str]:
        ...
        logger.info(
            f"Processing {len(csv_files)} TPIA CSV files for {self.jurisdiction}"
        )
        ...
        for csv_file in csv_files:
            ...
            if "_permits_" in csv_file.name:
                try:
                    ...
                    logger.debug(
                        f"Could not parse date from filename '{csv_file.name}'"
                    )
