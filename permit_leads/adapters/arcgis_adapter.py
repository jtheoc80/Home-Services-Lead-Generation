...
class ArcGISAdapter:
    ...
    def __init__(self, jurisdiction: Jurisdiction):
        ...
        self.feature_server = self.config.get("feature_server") or self.config.get(
            "url"
        )
        if not self.feature_server:
            raise ValueError(
                f"No feature_server or url found in config for {jurisdiction.name}"
            )
    ...
    def scrape_permits(
        ...
        try:
            ...
            logger.info(
                f"ArcGIS total count for {self.jurisdiction.name} since {since_str}: {total_count}"
            )
            ...
            logger.info(
                f"Using maxRecordCount: {max_record_count} for {self.jurisdiction.name}"
            )
            ...
            while records_fetched < total_count and (
                not limit or records_fetched < limit
            ):
                ...
                logger.debug(
                    f"Fetching batch: offset={offset}, count={records_to_fetch}"
                )
                ...
                if not data or "features" not in data:
                    logger.warning(
                        f"No features found in response for {self.jurisdiction.name}, offset {offset}"
                    )
                    break
                ...
                if not features:
                    logger.info(
                        f"No more features available for {self.jurisdiction.name}"
                    )
                    break
                ...
                logger.info(
                    f"Fetched {len(batch_permits)} permits from batch, total so far: {len(permits)}"
                )
            ...
            logger.info(
                f"Completed scraping {self.jurisdiction.name}: {len(permits)} permits collected"
            )
            return permits
