...
class ETLAwareArcGISAdapter:
    ...
    def scrape_permits(
        ...
        try:
            ...
            if since is None:
                since = self.etl_state.get_since_timestamp(
                    self.source_name, fallback_days=7
                )
    ...
    def _fetch_permits_from_arcgis(
        ...
        try:
            ...
            if "features" not in data:
                logger.warning(
                    f"No features found in ArcGIS response for {self.jurisdiction.name}"
                )
                return []
            ...
            logger.info(
                f"Successfully fetched {len(permits)} permits from {self.jurisdiction.name}"
            )
            return permits

        except Exception as e:
            logger.error(
                f"Error fetching from ArcGIS for {self.jurisdiction.name}: {e}"
            )
            raise
    ...
    def _parse_feature(self, feature: dict) -> Optional[PermitRecord]:
        ...
        for permit_field, source_field in self.field_map.items():
            if source_field in attributes:
                ...
                if (
                    permit_field
                    in ["issue_date", "application_date", "expiration_date"]
                    and value
                ):
                    ...
                    if isinstance(value, (int, float)) and value > 0:
                        try:
                            ...
                            mapped_data[permit_field] = datetime.fromtimestamp(
                                value / 1000
                            )
    ...
    def update_etl_state_after_upsert(self, num_records_processed: int = 0) -> bool:
        ...
        if success:
            logger.info(
                f"Updated ETL state for {self.source_name} after processing {num_records_processed} records: {current_time}"
            )
        else:
