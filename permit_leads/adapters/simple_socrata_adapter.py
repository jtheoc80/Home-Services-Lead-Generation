...
class SimpleSocrataAdapter(BaseAdapter):
    ...
    def __init__(self, cfg: Dict[str, Any], session=None):
        ...
        if (
            self.app_token
            and self.app_token.startswith("${")
            and self.app_token.endswith("}")
        ):
            ...
            env_vars = [
                "AUSTIN_SODA_APP_TOKEN",
                "AUSTIN_SOCRATA_APP_TOKEN",
                "SA_SOCRATA_APP_TOKEN",
                "DALLAS_SOCRATA_APP_TOKEN",
                "SOCRATA_APP_TOKEN",
                "SODA_APP_TOKEN",
            ]
            for env_var in env_vars:
                ...
                self.app_token = None
                logger.warning(
                    f"No Socrata app token found for {self.name}. Rate limits will be lower."
                )
        ...
        normalized.update(
            {
                ...
                or row.get(self.date_field)
                or "",
                "address": normalized.get("address")
                or row.get("address")
                or row.get("original_address1")
                or "",
