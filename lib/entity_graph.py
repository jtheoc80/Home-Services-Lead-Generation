...
class Firm(Entity):
    ...
    def _normalize_name(name: str) -> str:
        ...
        suffixes = [
            r'\s+LLC$', r'\s+INC$', r'\s+CORP$', r'\s+LTD$', r'\s+LP$', r'\s+LLP$',
            r'\s+CO$', r'\s+COMPANY$', r'\s+CORPORATION$', r'\s+INCORPORATED$',
            r'\s+LIMITED$', r'\s+PARTNERSHIP$'
        ]
...
class EntityGraph:
    ...
    def load_normalized_data(self) -> int:
        ...
        for category in ["permits", "violations", "inspections", "bids", "awards", "contractors"]:
            ...
            for file_path in files:
                try:
                    ...
                    total_loaded += loaded
                    logger.info(f"Loaded {loaded} {category} records from {file_path.name}")
    ...
    def _process_category_records(self, category: str, records: List[Dict[str, Any]]) -> int:
        ...
        for record in records:
            try:
                ...
                logger.warning(f"Error processing {category} record {record.get('id', 'unknown')}: {e}")
    ...
    def _process_inspection(self, record: Dict[str, Any]):
        ...
        permit_number = record.get("permit_number")
        if permit_number:
            related_permits = self._find_entities_by_attribute("permit", "permit_number", permit_number)
    ...
    def _get_or_create_firm(self, name: str, context_record: Dict[str, Any]) -> Optional[Firm]:
    ...
    def _find_entities_by_address(self, entity_type: str, address: str) -> List[str]:
        ...
        for entity_id, entity in self.entities.items():
            ...
            if entity_address:
                ...
                similarity = self._calculate_address_similarity(normalized_address, entity_normalized)
    ...
    def _find_entities_by_attribute(self, entity_type: str, attribute: str, value: str) -> List[str]:
    ...
    def _add_relationship(self, source_id: str, target_id: str, rel_type: str, attributes: Dict[str, Any]):
    ...
    def get_firm_profile(self, firm_id: str) -> Dict[str, Any]:
        ...
        total_permit_value = sum(
            float(self.entities[pid].attributes.get("value", 0) or 0)
            for pid in permits
        )
        
        total_award_value = sum(
            float(self.entities[aid].attributes.get("amount", 0) or 0)
            for aid in awards
        )
    ...
    def _get_related_entities(self, firm_id: str, rel_type: str, entity_type: str) -> List[str]:
    ...
    def get_network_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_permits": len([e for e in self.entities.values() if e.type == "permit"]),
            "total_violations": len([e for e in self.entities.values() if e.type == "violation"]),
            "total_awards": len([e for e in self.entities.values() if e.type == "award"]),
...
def main():
    ...
    for i, firm in enumerate(analysis['top_firms'][:5], 1):
        ...
        print(f"     Permits: {firm['permits']}, Awards: {firm['awards']}, Violations: {firm['violations']}")
